...
const AIRCRAFT_IMAGES: Record<string, string> = {
  C152: '/images/manufacturers/cessna/cessna-152.jpg',
  C172: '/images/manufacturers/cessna/cessna-cessna_172.jpg',
  C182: '/images/manufacturers/cessna/cessna-cessna_172.jpg',
  C208: '/images/manufacturers/cessna/cessna-152.jpg',
  PA28: '/images/manufacturers/piper/piper-pa28.jpg',
  PA44: '/images/manufacturers/piper/piper-pa28.jpg',
...
const getAircraftImage = (aircraftType: string, fallbackImage?: string): string => {
  ...
  return '/images/manufacturers/cessna/cessna-152.jpg';
};
...
export const DigitalLogbookPage: React.FC<DigitalLogbookPageProps> = ({ onBack, userProfile, embedded = false, inlineFullscreen = false }) => {
  ...
  const [hoveredLog, setHoveredLog] = useState<FlightLogEntry | null>(MOCK_FLIGHT_LOGS[0]);
  ...
  const previewLog = selectedLog || hoveredLog;
  ...
  const fetchFlightLogs = async () => {
    ...
    try {
      ...
      } else { setFlightLogs(MOCK_FLIGHT_LOGS); }
    } catch (error) {
  ...
  useEffect(() => {
    if (flightLogs.length > 0 && (!previewLog || !flightLogs.some((log) => log.id === previewLog.id))) {
      setHoveredLog(flightLogs[0]);
      setSelectedLog(null);
    }
  }, [flightLogs, previewLog]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const res = await callPilotApi<{ results: SearchProfileResult[] }>('searchProfiles', { query: crewSearchQuery.trim(), limit: 10 });
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', zIndex: 0, overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, rgba(100,116,139,0.18), rgba(30,41,59,0.3), rgba(2,6,23,0.5))' }} />
        <div style={{ position: 'absolute', inset: 0, backdropFilter: 'blur(1px)', background: 'rgba(15,23,42,0.08)' }} />
        ...
        <div style={{
          ...
          {flightLogs.length > 0 && (
            <div style={{
              ...
              <div style={{ width: '100%' }}>
                <p style={{ margin: '0 0 0.25rem', fontSize: '1.5rem', fontWeight: 800, color: '#ffffff', textAlign: 'center', letterSpacing: '-0.02em' }}>My Flightbook</p>
                  ...
                  {flightLogs.slice(0, 5).map((log) => (
                    ...
                    >
                      <div style={{
                        ...
                        border: selectedLog?.id === log.id ? '2px solid #0ea5e9' : '1px solid rgba(255, 255, 255, 0.1)',
                        ...
                        <div style={{
                          flex: '0 0 45%',
                          backgroundImage: `url('${getAircraftImage(log.aircraftType, log.image)}')`,
          ...
          {(
            <div style={{
              ...
              margin: useFullscreenLayout ? '0 clamp(1.5rem, 4vw, 3rem) 1.5rem clamp(1.5rem, 4vw, 3rem)' : '0',
              border: useFullscreenLayout ? '1px solid rgba(255, 255, 255, 0.14)' : 'none',
              boxShadow: useFullscreenLayout ? '0 32px 80px rgba(0,0,0,0.25), 0 0 0 1px rgba(255,255,255,0.08), inset 0 1px 0 rgba(255,255,255,0.18)' : 'none',
              ...
              {flightLogs.length > 0 && (
                <div style={{
                  width: '100%',
                  padding: useFullscreenLayout ? '1.5rem clamp(1.5rem, 4vw, 3rem) 0.75rem' : '1.5rem 0 0.75rem',
                  marginBottom: useFullscreenLayout ? '0.5rem' : '0',
                }}>
                  <div style={{
                      ...
                      {[
                        ...
                        <button
                          ...
                          style={{
                            ...
                            borderRadius: '10px',
                            border: 'none',
                            background: ledgerTab === tab.id ? 'rgba(14, 165, 233, 0.2)' : 'transparent',
                            color: ledgerTab === tab.id ? '#38bdf8' : '#94a3b8',
                            fontSize: '0.75rem',
                            fontWeight: 600,
                            cursor: 'pointer',
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
                      <div style={{
                        ...
                        <span className="sync-indicator-dot" style={{
                          width: '7px',
                          height: '7px',
                          borderRadius: '50%',
                          background: '#ef4444',
                      ...
                      {ledgerTab === 'byAircraft' && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          <span style={{ fontSize: '0.7rem', color: '#94a3b8', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Aircraft</span>
                          <select
                          ...
                          >
                            {aircraftTypes.map((type) => (
                              <option key={type} value={type} style={{ background: '#0f172a', color: '#ffffff' }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: useFullscreenLayout ? 'rgba(255, 255, 255, 0.04)' : 'rgba(255, 255, 255, 0.04)', borderBottom: useFullscreenLayout ? '1px solid rgba(255, 255, 255, 0.1)' : '2px solid rgba(255, 255, 255, 0.12)' }}>
                  ...
                  {loading ? (
                    ...
                    displayedLogs.map((log) => (
                      <tr
                        ...
                        style={{ borderBottom: useFullscreenLayout ? '1px solid rgba(255, 255, 255, 0.08)' : '1px solid rgba(255, 255, 255, 0.06)', cursor: 'pointer' }}
          ...
          {previewLog && (
            <div
              ...
              style={{
                ...
                borderRadius: useFullscreenLayout ? '0px' : '24px',
                overflow: 'hidden',
                background: useFullscreenLayout ? 'rgba(8, 8, 10, 0.95)' : 'rgba(255, 255, 255, 0.06)',
                border: useFullscreenLayout ? 'none' : '1px solid rgba(255, 255, 255, 0.14)',
                boxShadow: useFullscreenLayout ? '0 40px 100px rgba(0,0,0,0.5)' : '0 24px 60px rgba(0,0,0,0.35), 0 0 40px rgba(255,255,255,0.04), inset 0 1px 0 rgba(255,255,255,0.12)',
                maskImage: useFullscreenLayout ? 'linear-gradient(to bottom, rgba(0,0,0,1) 0%, rgba(0,0,0,1) 75%, rgba(0,0,0,0) 100%)' : 'none',
                WebkitMaskImage: useFullscreenLayout ? 'linear-gradient(to bottom, rgba(0,0,0,1) 0%, rgba(0,0,0,1) 75%, rgba(0,0,0,0) 100%)' : 'none',
            ...
            >
              <div style={{ display: 'flex', flexDirection: (useFullscreenLayout && !isMobile) ? 'row' : 'column', position: 'relative' }}>
                ...
                >
                  ...
                  >
                    <span style={{ fontSize: '0.75rem', fontWeight: 700, color: '#ffffff' }}>{previewLog.aircraft}</span>
                    <span style={{ fontSize: '0.7rem', color: '#94a3b8' }}>{previewLog.registration || '—'}</span>
                    <span style={{ fontSize: '0.7rem', color: '#fbbf24', fontWeight: 700 }}>A-★</span>
                  </div>
                  {(!useFullscreenLayout || inlineFullscreen) && (
                    <div style={{
                      ...
                      <p style={{ margin: '0 0 0.25rem', fontSize: '0.65rem', fontWeight: 800, letterSpacing: '0.15em', color: '#f472b6', textTransform: 'uppercase' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'end', justifyContent: 'space-between', gap: '1rem' }}>
                        <h3 style={{ margin: 0, fontSize: '1.75rem', fontWeight: 800, color: '#ffffff' }}>
                          {previewLog.aircraft}
                        </h3>
                        <span
                          style={{
                      ...
                      <p style={{ margin: '0.35rem 0 0', fontSize: '0.8rem', color: '#94a3b8' }}>
                        {previewLog.date} · {previewLog.route || '—'} · {previewLog.category || '—'}
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div style={{
                  ...
                  {isFullscreen && (
                    <div style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'end', justifyContent: 'space-between', gap: '1rem', marginBottom: '0.35rem' }}>
                        <h3 style={{ margin: 0, fontSize: '1.75rem', fontWeight: 800, color: '#ffffff' }}>
                          {previewLog.aircraft}
                        </h3>
                        <span
                  ...
                  <div style={{
                    ...
                    {[
                      { label: 'Mission Duration', value: `${previewLog.hours.toFixed(1)}h`, sub: 'Total flight time', pct: 75, color: '#fbbf24' },
                      { label: 'Block Time', value: `${(previewLog.hours * 1.25).toFixed(1)}h`, sub: 'Chocks off to on', pct: 100, color: '#38bdf8' },
                      { label: 'Landings', value: '3/3', sub: 'Successful', pct: 100, color: '#34d399', success: true },
                      { label: 'PIC Time', value: `${(previewLog.hours * 0.8).toFixed(1)}h`, sub: 'Pilot in command', pct: 60, color: '#a78bfa' },
                    ].map((stat) => (
                      <div
                        key={stat.label}
                        style={{
                      ...
                      >
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
                          <div>
                            <p style={{ margin: '0 0 0.15rem', fontSize: '0.55rem', fontWeight: 700, letterSpacing: '0.1em', color: '#64748b', textTransform: 'uppercase' }}>{stat.label}</p>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
                              <p style={{ margin: 0, fontSize: '1.25rem', fontWeight: 800, color: '#ffffff' }}>{stat.value}</p>
                              {stat.success && <span style={{ color: '#34d399', fontSize: '0.9rem' }}>✓</span>}
                            </div>
                          </div>
                          <p style={{ margin: 0, fontSize: '0.65rem', color: '#94a3b8' }}>{stat.sub}</p>
                        </div>
                        <div style={{ width: '100%', height: '4px', background: 'rgba(255,255,255,0.08)', borderRadius: '999px', overflow: 'hidden' }}>
                          <div style={{ width: `${stat.pct}%`, height: '100%', background: stat.color, borderRadius: '999px' }} />
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            <div style={{
              ...
              <div style={{ maxWidth: useFullscreenLayout ? '1200px' : '100%', margin: useFullscreenLayout ? '0 auto' : '0' }}>
                ...
                <div style={{
                  ...
                  <div>
                    <p style={{ margin: '0 0 0.35rem', fontSize: '0.7rem', fontWeight: 700, color: '#34d399' }}>Successes</p>
                    <p style={{ margin: 0, fontSize: '0.8rem', color: '#cbd5e1', lineHeight: 1.5 }}>
                      Excellent speed control and aim point management on approach. Precision in flaring and braking.
                    </p>
                  </div>
                  <div>
                    <p style={{ margin: '0 0 0.35rem', fontSize: '0.7rem', fontWeight: 700, color: '#fbbf24' }}>Areas for Refinement</p>
                    <p style={{ margin: 0, fontSize: '0.8rem', color: '#cbd5e1', lineHeight: 1.5 }}>
                      Rudder coordination at low speeds needs improvement during rotation. Aim for more consistent trim application.
                    </p>
                  </div>
                  <div>
                    <p style={{ margin: '0 0 0.35rem', fontSize: '0.7rem', fontWeight: 700, color: '#38bdf8' }}>Notes</p>
                    <p style={{ margin: 0, fontSize: '0.8rem', color: '#cbd5e1', lineHeight: 1.5 }}>
            ...
            <div style={{
              ...
              <div style={{ maxWidth: useFullscreenLayout ? '1200px' : '100%', margin: useFullscreenLayout ? '0 auto' : '0' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                  <p style={{ margin: 0, fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.1em', color: '#64748b', textTransform: 'uppercase' }}>Flight Crew Notary</p>
                  <button
                ...
                <div style={{
                  ...
                  {(flightCrew[previewLog.id] || []).length === 0 && !addingCrew && (
                    <p style={{ margin: 0, fontSize: '0.8rem', color: '#64748b', textAlign: 'center' }}>No crew recorded for this flight.</p>
                  )}

                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    {(flightCrew[previewLog.id] || []).map((crew) => (
                      <div key={crew.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.75rem' }}>
                        <div style={{
                          width: '44px',
                          height: '44px',
                          borderRadius: '50%',
                          background: crew.avatar ? `url(${crew.avatar}) center/cover` : 'linear-gradient(135deg, #38bdf8, #0ea5e9)',
                          display: 'flex',
                          alignItems: 'center',
                          justifyContent: 'center',
                          flexShrink: 0,
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem', marginBottom: '0.3rem' }}>
                            <p style={{ margin: 0, fontSize: '0.85rem', fontWeight: 700, color: '#ffffff' }}>{crew.name}</p>
                            <p style={{ margin: 0, fontSize: '0.65rem', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{crew.role}</p>
                          </div>
                          <div style={{
                            ...
                            <p style={{ margin: 0, fontSize: '0.8rem', color: '#e2e8f0', lineHeight: 1.5, wordBreak: 'break-word' }}>{crew.comment || 'No notary comment recorded.'}</p>
                          </div>
                        </div>
                      </div>
                    ))}
                  ...
                  {addingCrew && (
                    <div style={{ marginTop: '1.25rem', paddingTop: '1.25rem', borderTop: '1px solid rgba(255, 255, 255, 0.08)' }}>
                      <div style={{ position: 'relative', marginBottom: '0.75rem' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.6rem 0.75rem', borderRadius: '10px', border: '1px solid rgba(255, 255, 255, 0.1)', background: 'rgba(255, 255, 255, 0.03)' }}>
                          <Search size={14} color="#64748b" />
                          <input
                            type="text"
                            placeholder="Search PilotRecognition profiles..."
                            value={crewSearchQuery}
                            onChange={(e) => { setCrewSearchQuery(e.target.value); setSelectedCrewProfile(null); }}
                            style={{
                              flex: 1,
                              background: 'transparent',
                              border: 'none',
                          ...
                          />
                          {crewSearchLoading && <span style={{ fontSize: '0.65rem', color: '#64748b' }}>Searching…</span>}
                        </div>
                        {crewSearchResults.length > 0 && !selectedCrewProfile && (
                          <div style={{
                            position: 'absolute',
                            top: 'calc(100% + 0.4rem)',
                            left: 0,
                            ...
                            {crewSearchResults.map((profile) => (
                              <button
                                key={profile.id}
                                onClick={() => { setSelectedCrewProfile(profile); setCrewSearchQuery(profile.display_name || profile.full_name || ''); setCrewSearchResults([]); }}
                                style={{
                                  width: '100%',
                                  display: 'flex',
                                  alignItems: 'center',
                              ...
                              >
                                <div style={{
                                  ...
                                  height: '32px',
                                  borderRadius: '50%',
                                  background: profile.avatar_url || profile.profile_image_url ? `url(${profile.avatar_url || profile.profile_image_url}) center/cover` : 'linear-gradient(135deg, #38bdf8, #0ea5e9)',
                                  display: 'flex',
                                  alignItems: 'center',
                                  justifyContent: 'center',
                                  flexShrink: 0,
                                  ...
                                  {!(profile.avatar_url || profile.profile_image_url) && (profile.display_name || profile.full_name || '?').charAt(0)}
                                </div>
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  <p style={{ margin: 0, fontSize: '0.75rem', fontWeight: 700, color: '#ffffff', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{profile.display_name || profile.full_name || 'Unknown'}</p>
                                  <p style={{ margin: 0, fontSize: '0.6rem', color: '#64748b', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{profile.email || profile.id}</p>
                                </div>
                              </button>
                            ))}
                          </div>
                      ...
                      {selectedCrewProfile && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.75rem', padding: '0.5rem 0.75rem', borderRadius: '10px', background: 'rgba(56, 189, 248, 0.08)', border: '1px solid rgba(56, 189, 248, 0.18)' }}>
                          <div style={{
                            width: '32px',
                            height: '32px',
                            borderRadius: '50%',
                            background: selectedCrewProfile.avatar_url || selectedCrewProfile.profile_image_url ? `url(${selectedCrewProfile.avatar_url || selectedCrewProfile.profile_image_url}) center/cover` : 'linear-gradient(135deg, #38bdf8, #0ea5e9)',
                            display: 'flex',
                            alignItems: 'center',
                            justifyContent: 'center',
                            flexShrink: 0,
                            ...
                            {!(selectedCrewProfile.avatar_url || selectedCrewProfile.profile_image_url) && (selectedCrewProfile.display_name || selectedCrewProfile.full_name || '?').charAt(0)}
                          </div>
                          <p style={{ margin: 0, fontSize: '0.8rem', fontWeight: 700, color: '#38bdf8' }}>{selectedCrewProfile.display_name || selectedCrewProfile.full_name || 'Selected profile'}</p>
                          <button
                            onClick={() => { setSelectedCrewProfile(null); setCrewSearchQuery(''); }}
                            style={{ marginLeft: 'auto', padding: '0.25rem 0.5rem', borderRadius: '6px', border: 'none', background: 'transparent', color: '#94a3b8', fontSize: '0.65rem', cursor: 'pointer' }}
                          >
                            Change
                          </button>
                        </div>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem', flexWrap: isMobile ? 'wrap' : 'nowrap' }}>
                        ...
                        <button
                          onClick={() => {
                            const name = selectedCrewProfile?.display_name || selectedCrewProfile?.full_name || crewSearchQuery.trim();
                            if (!name) return;
                            setFlightCrew((prev) => ({
                              ...prev,
                              [previewLog.id]: [
                                ...(prev[previewLog.id] || []),
                                {
                                  ...
                                  avatar: selectedCrewProfile?.avatar_url || selectedCrewProfile?.profile_image_url,
          ...
          {previewLog && (
            <div style={{
              ...
              <div style={{ maxWidth: useFullscreenLayout ? '1200px' : '100%', margin: useFullscreenLayout ? '0 auto' : '0' }}>
                <p style={{ margin: '0 0 0.75rem', fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.1em', color: '#64748b', textTransform: 'uppercase' }}>Recognition AI</p>
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '1rem' : '1.5rem', flexWrap: 'wrap', justifyContent: isMobile ? 'center' : 'flex-start' }}>
                {[
                  ...
                  { label: 'Total Dual', value: `${(totalHours * 0.35).toFixed(1)}h`, color: '#fbbf24' },
                  { label: 'Total IFR', value: `${(totalHours * 0.25).toFixed(1)}h`, color: '#34d399' },
                ].map((stat) => (
                  <div key={stat.label} style={{ textAlign: 'center' }}>
                    <p style={{ margin: '0 0 0.15rem', fontSize: '0.55rem', fontWeight: 700, letterSpacing: '0.08em', color: '#64748b', textTransform: 'uppercase' }}>{stat.label}</p>
