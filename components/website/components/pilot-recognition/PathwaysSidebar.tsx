...
import { Target, TrendingUp, Award, Compass, Zap, Route, Star } from 'lucide-react';
...
export const PathwaysSidebar: React.FC<PathwaysSidebarProps> = ({
  ...
  const profileData = {
    name:
      (currentUser as { user_metadata?: { full_name?: string } } | null)?.user_metadata
        ?.full_name || 'Benjamin Bowler',
    totalHours: '1,247',
    photoUrl:
      (currentUser as { user_metadata?: { avatar_url?: string } } | null)?.user_metadata
        ?.avatar_url || null,
    initials: 'BB',
  };
  ...
  const navItems = [
    ...
    {
      id: 'airline-expectations' as const,
      label: 'Airline Expectations',
      page: currentUser ? 'portal-airline-expectations' : 'airline-expectations',
    },
    ...
    {
      id: 'aviation-authorities' as const,
      label: 'Global Aviation Authorities',
      page: 'global-aviation-authorities',
    },
  ];
    ...
    >
      ...
      <nav style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem', flexShrink: 0 }}>
        {navItems.map((item) => {
          ...
          const isActive = isVaultItem ? activeSection === 'wallet' : activeSection === item.id;
          return (
            <button
              key={item.id}
              onClick={() => {
              ...
              style={{
                ...
                border: isVaultItem
                  ? `2px solid ${isActive ? '#dc2626' : 'rgba(220,38,38,0.4)'}`
                  : 'none',
            ...
            >
              ...
              {isActive && (
                <svg
                  ...
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke={isVaultItem ? '#dc2626' : 'currentColor'}
                  strokeWidth="3"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                >
      ...
      >
        ...
        <div style={{ marginBottom: '0.5rem' }}>
          ...
          >
            ...
            <span style={{ fontSize: '0.9rem', fontWeight: 700, color: scoreColor }}>
              {prScore}
            </span>
          </div>
          <div
          ...
          <p style={{ margin: '0.25rem 0 0', fontSize: '0.65rem', color: '#64748b' }}>
            {prScore >= 80
              ? 'Excellent standing for top airlines'
              : prScore >= 60
                ? 'Good match for major carriers'
        ...
        >
          ...
          <p
            style={{ margin: '0 0 0.25rem', fontSize: '0.9rem', fontWeight: 600, color: '#ffffff' }}
          >
            {topPathway}
          </p>
        ...
        >
          ...
          <p style={{ margin: 0, fontSize: '0.85rem', fontWeight: 500, color: '#ffffff' }}>
            {topAirline}
          </p>
          <p style={{ margin: '0.25rem 0 0', fontSize: '0.65rem', color: '#64748b' }}>
            Based on your profile alignment
          </p>
        </div>
      </div>
