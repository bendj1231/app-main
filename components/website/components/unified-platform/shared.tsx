...
export const LogbookPreviewPanel: React.FC<{ profile: any; onOpenLogbook: () => void }> = ({ profile, onOpenLogbook }) => {
  const [logs, setLogs] = React.useState<any[]>([]);
  React.useEffect(() => {
    const id = profile?.id;
    if (!id) return;
    let active = true;
    supabase.from('pilot_flight_logs').select('id,date,aircraft_type,route,hours').eq('user_id', id).order('date', { ascending: false }).limit(3).then(({ data }) => {
...
export const CredentialRequestCard: React.FC<{ request: any; onRespond: (approved: boolean) => Promise<void> }> = ({ request, onRespond }) => {
...
export const NotificationsFeedPanel: React.FC<{ profileId?: string; profile?: any }> = ({ profileId, profile }) => {
  ...
  React.useEffect(() => {
    if (!profileId) return;
    let active = true;
    supabase.from('pilot_notifications').select('*').eq('pilot_id', profileId).order('created_at', { ascending: false }).limit(8).then(({ data }) => {
...
export const SectionCard: React.FC<{ title: string; children: React.ReactNode; className?: string; action?: React.ReactNode; style?: React.CSSProperties }> =
...
export const EmailVerifyGate: React.FC<{ onResend: () => void; sent: boolean }> = ({ onResend, sent }) => (
