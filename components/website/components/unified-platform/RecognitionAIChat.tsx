    ...
    try {
      const response = await fetch('https://pilotrecognition-api.benjamintigerbowler.workers.dev/api', {
        ...
        body: JSON.stringify({
          ...
          params: {
            message,
            user_id: (profile?.id || profile?.user_id || null) as string | null,
            last_topic: lastTopic,
            profile_context: {
              first_name: profile?.first_name || profile?.name || null,
              license_type: profile?.license_type || null,
              total_flight_hours: (profile?.total_flight_hours || profile?.total_hours || null) as number | null,
              medical_class: profile?.medical_class || null,
              elp_level: profile?.elp_level || null,
              career_goal: profile?.career_goal || null,
              ato_name: profile?.ato_name || null,
              verified_account: profile?.verified_account === true,
            },
            pathways_context: [],
          },
        }),
      ...
      if (!response.ok) {
        ...
        if (String(errorMessage).includes('Daily AI chat limit reached')) {
