...
import { MessageSquare, Bell, Settings, Menu, LogOut, Lock } from 'lucide-react';
...
export const PlatformNavbar: React.FC<PlatformNavbarProps> = ({
  ...
  const tcUpdatePending = useMemo(() => {
    const CURRENT_TC_VERSION = 'v2-2026';
    return !!(userProfile?.consent_version && userProfile.consent_version !== CURRENT_TC_VERSION);
  }, [userProfile]);
  ...
  useEffect(() => {
    ...
    const fetchNotifs = async () => {
      const allNotifs = await callApi<Notification[]>('queryTable', {
  ...
  return (
    <div
      className="platform-navbar fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 md:px-8 lg:px-10"
      style={{
        height: '68px',
        background: transparent
          ? 'transparent'
          : 'linear-gradient(to bottom, #172554 0%, #0f172a 100%)',
        backdropFilter: transparent ? 'none' : 'none',
        WebkitBackdropFilter: transparent ? 'none' : 'none',
      ...
      {currentPage !== 'home' && (
        <React.Fragment>
          ...
          <div className="hidden lg:flex flex-1 items-center justify-center min-w-0 mx-2 md:mx-4">
            <div
              className={`flex items-center gap-1 overflow-hidden ${transparent ? 'px-3 py-1.5 rounded-full border border-white/10' : ''}`}
              style={
                transparent
                  ? {
          ...
          <>
            <div className="flex items-center gap-2">
              ...
              <div className="relative" ref={bellRef}>
                ...
                >
                  ...
                  {isProfileComplete && (notifCount > 0 || tcUpdatePending || !emailVerified) && (
                    <span
                      ...
                      style={{ background: '#f59e0b', border: '1.5px solid rgba(15,22,35,0.9)' }}
                    >
                ...
                {bellOpen && (
                  <div
                    className="absolute right-0 top-12 w-80 z-50 shadow-2xl"
                    style={{
                    ...
                    {!isProfileComplete ? (
                      <div className="px-5 py-6 text-center">
                        ...
                        <p className="text-[11px] text-white/50 leading-relaxed mb-3">
                          Add your name, license type, and occupation to unlock notifications and
                          pathway alerts.
                        </p>
                        <div className="space-y-1.5 text-left max-w-xs mx-auto">
                          ...
                          >
                            <span
                              className={`text-[8px] font-black w-4 h-4 rounded-full flex items-center justify-center ${userProfile?.full_name ? 'bg-emerald-500/20 text-emerald-400' : 'bg-white/10 text-white/40'}`}
                            >
                          ...
                          >
                            <span
                              className={`text-[8px] font-black w-4 h-4 rounded-full flex items-center justify-center ${userProfile?.current_occupation ? 'bg-emerald-500/20 text-emerald-400' : 'bg-white/10 text-white/40'}`}
                            >
                              {userProfile?.current_occupation ? '✓' : '2'}
                            </span>
                          ...
                          >
                            <span
                              className={`text-[8px] font-black w-4 h-4 rounded-full flex items-center justify-center ${userProfile?.license_type ? 'bg-emerald-500/20 text-emerald-400' : 'bg-white/10 text-white/40'}`}
                            >
                              {userProfile?.license_type ? '✓' : '3'}
                            </span>
                      ...
                      <>
                        ...
                        <button
                          ...
                          className="w-full px-4 py-2.5 text-[10px] font-black tracking-wider text-sky-400 hover:text-sky-300 border-t border-white/5 text-center transition-colors"
                        >
              ...
              <button
                ...
                style={{
                  ...
                  height: 44,
                  background:
                    currentPage === 'settings' ? 'rgba(75,85,99,0.95)' : 'rgba(55,65,81,0.85)',
                  border:
