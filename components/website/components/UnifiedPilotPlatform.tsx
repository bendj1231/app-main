  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col gap-4">
        ...
        {/* ── Quick Actions ── */}
        <div>
          <p className="text-[10px] uppercase tracking-widest text-white/30 font-bold mb-2">Quick Actions</p>
          <div className="grid grid-cols-3 gap-2">
            {[
              { label: 'Log Hours', icon: Clock, tab: 'logbook' as TabId },
              { label: 'Browse Pathways', icon: Map, tab: 'pathways' as TabId },
              { label: 'Start Program', icon: BookOpen, tab: 'programs' as TabId },
            ].map(a => {
              const Icon = a.icon;
              return (
                <button key={a.label} onClick={() => setTab(a.tab)}
                  className="flex flex-col items-center gap-1.5 py-3 rounded-xl text-white/60 hover:text-white transition-all hover:brightness-110"
                  style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' }}
                >
                  <Icon size={16} />
                  <span className="text-[10px] font-bold tracking-wide">{a.label.toUpperCase()}</span>
                </button>
              );
            })}
          </div>
        ...
        {/* ── Pathway Match Preview ── */}
        <div className="rounded-xl overflow-hidden" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
          <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            <div>
              <p className="text-xs font-bold text-white tracking-wide">Pathway Matches</p>
              <p className="text-[10px] text-white/40">{airlines.length > 0 ? `${Math.min(airlines.length, 3)} airlines match your current profile` : 'Add flight hours to unlock matches'}</p>
            </div>
            <button onClick={() => setTab('pathways')} className="text-[10px] font-bold text-sky-400 hover:text-sky-300 flex items-center gap-1">VIEW ALL <ChevronRight size={10} /></button>
          </div>
          <div className="divide-y" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
            {airlines.slice(0, 2).map((airline: any) => (
              <div key={airline.id} className="flex items-center gap-3 px-4 py-2.5">
                {airline.logo_url
                  ? <img src={airline.logo_url} alt={airline.name} className="w-7 h-7 object-contain rounded flex-shrink-0" />
                  : <div className="w-7 h-7 rounded bg-white/10 flex items-center justify-center flex-shrink-0"><Plane size={12} className="text-white/50" /></div>
                }
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-bold text-white truncate">{airline.name}</p>
                  <p className="text-[10px] text-white/40">{airline.minimum_hours ? `Min. ${airline.minimum_hours} hrs` : airline.country ?? ''}</p>
                </div>
                <span className="text-[10px] px-2 py-0.5 rounded-full font-bold" style={{ background: 'rgba(16,185,129,0.15)', color: '#6ee7b7', border: '1px solid rgba(16,185,129,0.2)' }}>MATCH</span>
              </div>
            ))}
            {airlines.length > 2 && (
              <button onClick={() => setTab('pathways')} className="w-full flex items-center justify-center gap-1.5 py-2.5 text-[10px] font-bold text-white/30 hover:text-white/60 transition-colors">
                <Lock size={9} /> {airlines.length - 2} more — Recognition+ required
              </button>
            )}
            {airlines.length === 0 && (
              <div className="px-4 py-4 text-center">
                <p className="text-[10px] text-white/30">Log flight hours and verify credentials to unlock pathway matches.</p>
              </div>
            )}
          </div>
        </div>
        ...
        {(hours === 0 || score === 0) && (
          <div className="rounded-xl overflow-hidden" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
            <div className="px-4 py-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              <p className="text-xs font-bold text-white tracking-wide">Get Started</p>
              <p className="text-[10px] text-white/40">Complete these steps to activate your Recognition Score</p>
            </div>
            <div className="divide-y" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
              {[
                { step: 1, label: 'Create your profile', done: !!profile, tab: 'profile' as TabId },
                { step: 2, label: 'Log your first flight hours', done: hours > 0, tab: 'logbook' as TabId },
                { step: 3, label: 'Verify credentials in Wallet', done: walletChecks.some(c => c.status === 'verified'), tab: 'wallet' as TabId },
                { step: 4, label: 'Submit pathway interest', done: false, tab: 'pathways' as TabId },
                { step: 5, label: 'Start Foundation Program', done: enrolledInFoundation, tab: 'programs' as TabId },
              ].map(item => (
                <button key={item.step} onClick={() => !item.done && setTab(item.tab)}
                ...
                >
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 text-[10px] font-black ${
                    item.done ? 'bg-emerald-500/30 text-emerald-400' : 'bg-white/10 text-white/50'
                  }`}>
                    {item.done ? '✓' : item.step}
                  </div>
                  <span className={`text-xs font-semibold ${item.done ? 'line-through text-white/30' : 'text-white/70'}`}>{item.label}</span>
                  {!item.done && <ChevronRight size={10} className="text-white/20 ml-auto" />}
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="rounded-xl overflow-hidden" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
          <div className="px-4 py-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            <p className="text-xs font-bold text-white tracking-wide">Recent Activity</p>
          </div>
          <div className="divide-y" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
            {[
              { icon: User, label: 'Profile created', time: 'Today', colour: 'text-sky-400' },
              { icon: Shield, label: 'Credential Wallet set up', time: 'Today', colour: 'text-indigo-400' },
              { icon: Star, label: 'Recognition Score activated', time: 'Today', colour: 'text-orange-400' },
            ].map((item, i) => {
              const Icon = item.icon;
              return (
                <div key={i} className="flex items-center gap-3 px-4 py-2.5">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${item.colour}`} style={{ background: 'rgba(255,255,255,0.06)' }}>
                    <Icon size={11} />
                  </div>
                  <p className="text-xs text-white/60 flex-1">{item.label}</p>
                  <p className="text-[10px] text-white/25 flex-shrink-0">{item.time}</p>
                </div>
              );
            })}
            {hours === 0 && score === 0 && (
              <div className="px-4 py-3 text-center">
                <p className="text-[10px] text-white/25">Flight hours and program activity will appear here.</p>
              </div>
            )}
          </div>
        </div>
