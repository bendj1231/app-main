...
export const TopNavbar: React.FC<TopNavbarProps> = ({
  ...
  const [domainBrand, setDomainBrand] = useState<'recognition' | 'shortage' | 'careerpathways'>(
    'recognition'
  );
          ...
          if (data) {
            ...
            setIsEnrolledInFoundation(
              Array.isArray(data.enrolled_programs) &&
                data.enrolled_programs.includes('Foundational')
            );
          } else {
  ...
  // Show graphics tooltip on first visit (logged in or out), auto-dismiss after 4s
  useEffect(() => {
    ...
    },
    {
      ...
      subItems: [
        ...
        },
        {
          ...
          target: '/ebt-cbta',
          bullets: ['Competency-based training', 'Evidence-based assessment', 'Interview priority'],
        },
        {
          category: 'Training & Development',
  ...
  const visibleNavItems =
    currentPage === 'home' ? navItems.filter((item) => item.name !== 'Home') : navItems;
  ...
  return (
    <>
      ...
      <nav
        className={`fixed top-10 left-0 right-0 z-[200] transition-all duration-500 ${
          isLight
        ...
        >
          ...
          >
            <div className="flex items-baseline transition-all duration-300 group-hover:scale-105">
              <span
                className={`${
                  (isLight && passedPathwayGrid) ||
                  (isDark && scrolled) ||
                  (!passedPathwayGrid && scrolled)
                    ? 'text-black'
                    : 'text-white'
              ...
              >
                {domainBrand === 'shortage' ? (
                  ...
                  <>
                    ...
                    <span
                      className={`relative ${(isLight && passedPathwayGrid) || (isDark && scrolled) || (!passedPathwayGrid && scrolled) ? 'text-black' : 'text-white'}`}
                    >
                      .org
                  ...
                  <>
                    ...
                    >
                      ...
                      {countryCode && (
                        <sup
                          className={`absolute top-0 -right-2 text-[8px] font-bold leading-none ${(isLight && passedPathwayGrid) || (isDark && scrolled) || (!passedPathwayGrid && scrolled) ? 'text-slate-400' : 'text-white/50'}`}
                        >
                          {countryCode}
                        </sup>
                      )}
              ...
              {currentUser && (
                <div className="flex gap-4 mt-4">
                  <div className="flex-1 flex items-center justify-center gap-2">
                    <div className="relative">
                      <div
                        className="w-12 h-16 rounded-[50%/40%] bg-slate-700 flex items-center justify-center overflow-hidden"
                        style={{ borderRadius: '45% / 50%' }}
                      >
                        {profileImageUrl ? (
                          <img
