...
const dummyCards = [
  ...
  },
  {
    ...
    dynamicSubtitles: [
      '50+ hours mentorship. Start your journey today!',
      'Match your profile to airline standards',
      'Find your perfect aircraft type rating',
      'AI matches you with your dream airline job',
      'From student to captain - your complete pathway',
    ],
...
const heroSlides: HeroSlide[] = [
  {
    ...
    titleSuffix: '+',
    description:
      'Turn your logbook into a verified credential meeting international standards. Verified flight hours, industry credibility, and the recognition every pilot deserves.',
            ...
            <AnimatePresence mode="wait" custom={direction}>
              ...
              >
                ...
                <p className="text-xs text-slate-300 leading-relaxed max-w-[220px] ml-auto">
                  ...
                  <span className="text-red-500">{slide.rightHighlightText}</span>
                </p>
              </motion.div>
            </AnimatePresence>
          </div>
...
export const PathwayGrid: React.FC<PathwayGridProps> = ({
  ...
  const currentAccentColor =
    viewSets.find((v) => v.id === currentViewKey)?.accentColor || 'bg-yellow-400';
  ...
  return (
    <div className="relative z-40 flex flex-col items-center justify-start pt-0 pb-10 px-3 sm:px-4 md:px-8 lg:px-12 xl:px-16 pointer-events-auto w-full">
      <div
                    ...
                    <div className="grid grid-cols-1 gap-2 mb-4">
                      ...
                      {currentCards.slice(1, 5).map((card, idx) => (
                        <motion.div
                          key={card.id}
                          variants={cardVariants}
                          className={idx < 1 ? 'h-[200px] xs:h-[220px]' : 'h-[120px] xs:h-[130px]'}
                        >
                          <GridCard
                            card={card}
                  ...
                  {!isMobileView && !isTabletView && (
                    <>
                      ...
                      <motion.div
                        key={currentCards[0].id}
                        variants={cardVariants}
                        className="mb-6 h-[280px] lg:h-[320px] xl:h-[340px] 2xl:h-[380px]"
                      >
                        <AccessPlatformCard
                          onLogin={onLogin || (() => {})}
                          onNavigate={onNavigate}
                      ...
                      <div className="grid grid-cols-3 gap-4 md:gap-6">
                        {currentCards.slice(2, 5).map((card) => (
                          <motion.div
                            key={card.id}
                            variants={cardVariants}
                            className="h-[185px] lg:h-[210px] xl:h-[225px] 2xl:h-[245px]"
                          >
                            <GridCard
                              card={card}
                              isHovered={hoveredCard === card.id}
...
const GridCard: React.FC<GridCardProps> = ({
  ...
  const displayTitleOverride =
    card.id === 'programs' ? 'Discover Pilot Programs' : finalDisplayTitle;
    ...
    >
      ...
      {card.isDirectory ? (
        ...
        >
          ...
          {isLargeCard && (
            <div
              className={`absolute bottom-0 left-0 right-0 p-3 md:p-4 z-20 transition-colors duration-300 ${card.isLightMode ? 'bg-white border-t border-slate-200' : isMsfsSelected ? 'bg-[#00b4d8]' : 'bg-[#111827]'} ${card.id === 'w1000-suite' ? 'pb-12' : ''}`}
            >
              <div className="flex flex-col">
                {/* Title row with double chevrons - MSFS style */}
                <div className="flex items-center gap-1.5 mb-1">
                  <span
                    className={`text-xs md:text-sm font-bold ${card.isLightMode ? 'text-slate-700' : isMsfsSelected ? 'text-white' : 'text-white/80'}`}
                  >
                    &#8811;
                  </span>
                  <h3
                    className={`text-xs md:text-sm font-bold uppercase tracking-wider ${card.isLightMode ? 'text-slate-900' : 'text-white'} ${card.id === 'credentials' ? 'text-black' : ''}`}
                  >
                    {card.id === 'pilot-recognition' ? (
                      <>
                        Learn more about <span className="text-red-600">Recognition+</span>
                      </>
                    ) : card.id === 'pathways' ? (
                      <>
                      ...
                      <>
                        Discover Pilot <span className="text-red-500">Programs</span>
                      </>
                    ) : (
                      finalDisplayTitle
                    )}
                ...
                <div
                  className={`w-full max-w-[120px] h-1 mb-2 ${card.isLightMode ? 'bg-gradient-to-r from-slate-400 to-transparent' : isMsfsSelected ? 'bg-gradient-to-r from-white to-transparent' : 'bg-gradient-to-r from-[#00b4d8] to-transparent'}`}
                />
                {/* Description - dark text for light mode, light text for dark */}
                <p
                  className={`text-[10px] md:text-xs leading-tight line-clamp-3 ${card.isLightMode ? 'text-slate-600' : isMsfsSelected ? 'text-white/85' : 'text-slate-300'}`}
                >
                  {finalDisplaySubtitle}
                </p>
          ...
          {!isLargeCard && (
            <div className="absolute bottom-0 left-0 right-0">
              ...
              >
                ...
                <div
                  className={`absolute top-0 left-0 w-10 h-[2px] ${isMsfsSelected ? 'bg-white' : 'bg-[#00b4d8]'}`}
                />
                <div className="flex items-center gap-2">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1 mb-0.5">
                      <span
                        className={`text-[10px] md:text-xs font-bold ${isMsfsSelected ? 'text-white' : 'text-white/80'}`}
                      >
                        &#8811;
                      </span>
                    ...
                    <p
                      className={`text-[9px] md:text-[10px] line-clamp-3 leading-tight ${isMsfsSelected ? 'text-white/85' : 'text-slate-300'}`}
                    >
                      {finalDisplaySubtitle}
                    </p>
                  </div>
