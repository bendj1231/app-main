...
const getViewCards = (isLoggedIn: boolean, _isEnrolledInFoundation: boolean = false) => ({
  home: [
    {
      id: 'get-started-recognition',
      image:
        'https://res.cloudinary.com/dridtecu6/image/upload/v1776948158/sedmmczhyibdw1okfcgx.png',
      title: 'Get Started with Recognition+',
      subtitle:
        'Launch your Recognition Profile and begin building verified operator-ready credentials.',
      icon: Compass,
      badge: 'NEW',
      accentColor: 'from-red-500/80 to-pink-400/80',
    },
    {
      id: 'card-2',
      images: [
        '/images/set-06-pathways/pathway2.png',
        '/images/airline-operations.png',
      ...
      dynamicTitles: [
        'Are You Eligible for Singapore Airlines?',
        'Stop Sending CVs Into Black Holes',
        'Run a Live Compliance Audit Now',
      ],
      dynamicSubtitles: [
        'Operators worldwide are actively filtering for verified flight crews. See exactly how your credentials line up.',
        'Unverified applications get filtered out before a human ever reads them. Change that today.',
        'Check your route eligibility against live fleet requirements — hours, ratings, EBT scores and more.',
      ],
      subtitle:
        'Operators worldwide are actively filtering for verified flight crews. Stop sending unverified CVs into black holes.',
      loggedInSubtitle:
        'Live profile matched against verified airline requirements — see your exact gaps',
      enrolledSubtitle:
        'Live profile matched against verified airline requirements — see your exact gaps',
      icon: null,
      badge: 'New',
  ...
  programs: [
    {
      id: 'foundation',
      videoUrl:
        'https://pub-7d91692145fd4db8ad180eaf345568f7.r2.dev/videos/My-Movie-3-720WebShareName.mov',
  ...
  pathways: [
    ...
    },
    {
      ...
      image:
        'https://res.cloudinary.com/dridtecu6/image/upload/v1776690048/pathways/cadet-programs.jpg',
    ...
    },
    {
      ...
      image:
        'https://res.cloudinary.com/dridtecu6/image/upload/v1776689980/pathways/air-taxi-evtol.jpg',
...
const heroSlides: HeroSlide[] = [
  {
    ...
    rightTitle: 'Connecting pilots ',
    rightTitleHighlight: 'to the industry',
    rightDescription:
      'Align your profile with specific career paths. Discover posted expectations & requirements from manufacturers, airlines, and the wider aviation industry before submitting your interest in a pathway.',
    rightHighlightText:
      'Gain exclusive access to private and eVTOL pathways for serious pilots with Recognition+.',
    bgImage: '/images/set-07-ui-graphics/123.png',
    mobileBgImage: '/images/set-07-ui-graphics/123.png',
    mobileHeroBackground:
      'https://images.unsplash.com/photo-1474302770737-17365221f888?auto=format&fit=crop&w=800&q=80',
    mobileDescription: 'Turn your logbook into a verified credential that airlines trust.',
    mobileRightDescription:
      'Align your profile with career paths from airlines and industry partners.',
  },
];
...
const AccessPlatformCard: React.FC<{
  ...
  const featureCards = [
    {
      label: 'Available now',
      title: 'Turn your logbook into a verified credential that airlines trust.',
      cta: 'Discover more',
    ...
    },
    {
      ...
      title: 'Compare airline requirements and explore type ratings matched to your profile.',
      cta: 'Discover more',
    ...
    },
    {
      ...
      title: 'Structured training pathways from flight school to airline-ready professional.',
      cta: 'Discover more',
    ...
    },
    {
      ...
      title: 'Join verified pilot discussions and access flight deck tools built for aviators.',
      cta: 'Discover more',
      target: 'https://pilotterminal.com',
      external: true,
    },
  ];
  ...
  // Auto-cycle feature card every 4 seconds
  useEffect(() => {
    const interval = setInterval(() => {
  ...
  const feature = featureCards[featureIndex];
  ...
  return (
    ...
    >
      ...
      {isMobile ? (
        ...
        <>
          {/* Full-bleed background image — hangar / aviation scene */}
          <AnimatePresence mode="wait">
            <motion.div
              key={slide.id}
              className="absolute inset-0"
            ...
            >
              <img
                src={slide.mobileHeroBackground || slide.mobileBgImage || slide.bgImage}
          ...
          <div
            ...
            style={{
              background:
                'linear-gradient(to bottom, rgba(2,6,23,0.82) 0%, rgba(2,6,23,0.55) 30%, rgba(2,6,23,0.45) 55%, rgba(2,6,23,0.75) 100%)',
            }}
          />

          {/* Hero content — Airbus-style: headline top-left + bottom info card */}
          <div className="absolute inset-0 flex flex-col justify-between px-5 pt-20 pb-5 overflow-y-auto">
            {/* Top-left headline */}
            <div className="relative z-10 w-full">
              <h1 className="text-white text-3xl sm:text-4xl font-bold leading-[1.1] tracking-tight max-w-[280px]">
                {slide.title}
            ...
            {/* Bottom info card — shuffling features carousel */}
            <div className="relative z-10 w-full">
              <AnimatePresence mode="wait">
                <motion.button
                  key={featureIndex}
                  ...
                  onClick={() =>
                    feature.external
                      ? window.open(feature.target, '_blank', 'noopener,noreferrer')
                      : onNavigate(feature.target)
                  }
                  className="w-full text-left bg-[#0d1b3e]/95 backdrop-blur-md rounded-lg p-4 border border-white/10 shadow-2xl transition-transform active:scale-[0.99]"
                >
              ...
              <div className="mt-3 flex items-center gap-3">
                <button
                  ...
                  className="flex-1 py-3 rounded-full text-sm font-bold uppercase tracking-wider text-white transition-all active:scale-95"
                  style={{
                ...
                <button
                  ...
                  className="flex-1 py-3 rounded-full text-sm font-bold uppercase tracking-wider bg-white text-slate-900 border border-white/40 hover:bg-slate-100 transition-all active:scale-95"
                >
                  Learn More
                </button>
              </div>
...
export const PathwayGrid: React.FC<PathwayGridProps> = ({
  ...
  const { currentUser, userProfile, refreshUserProfile: _refreshUserProfile } = useAuth();
  const { callApi: _callApi } = useWorkerAuth();
  const isLoggedIn = !!currentUser;

  // Robust enrollment check - same logic as portal dashboard
  const isEnrolledInFoundation =
    userProfile?.enrolled_programs && Array.isArray(userProfile.enrolled_programs)
      ? userProfile.enrolled_programs.some(
          (p: string) =>
  ...
  const [viewMode, _setViewMode] = useState<'auto' | 'mobile' | 'desktop'>('auto');
  const [direction, setDirection] = useState(0);
  ...
  // Responsive breakpoints: mobile <768
  ...
  const getCardClickHandler = (card: GridCardData) => {
    return () => {
      ...
      const navMap: Record<string, string> = {
        'FOUNDATION-PROGRAM-ENROLL': !isLoggedIn
          ? 'become-member'
          : isEnrolledInFoundation
            ? 'platform?tab=programs'
            : 'platform?tab=programs',
  ...
  return (
    <div className="relative z-40 flex flex-col items-center justify-start pt-0 pb-0 md:pb-10 px-0 md:px-16 pointer-events-auto w-full">
      <div
        ref={gridInteractionRef}
        className="relative w-full md:max-w-[1200px] md:mx-auto"
                  ...
                  {isMobileView ? (
                    <div className="w-full flex flex-col gap-4">
                      ...
                      <motion.div
                        ...
                        className="relative w-full h-[calc(100vh-80px)] min-h-[420px] rounded-none"
                      >
                        <AccessPlatformCard
                          isMobile
                          onLogin={onLogin || (() => {})}
                  ...
                  {isMobileView ? (
                    <div className="grid grid-cols-1 gap-2 mb-4">
                      ...
                      >
                        <GridCard
                          card={currentCards[0]}
                          isHovered={hoveredCard === currentCards[0]?.id}
                          onHover={() => setHoveredCard(currentCards[0]?.id || null)}
                          onLeave={() => setHoveredCard(null)}
                          onClick={getCardClickHandler(currentCards[0])}
                          onNavigate={onNavigate}
                          className="w-full h-full"
                          isLoggedIn={isLoggedIn}
                          isEnrolledInFoundation={isEnrolledInFoundation}
                      ...
                      >
                        <GridCard
                          card={currentCards[1]}
                          isHovered={hoveredCard === currentCards[1]?.id}
                          onHover={() => setHoveredCard(currentCards[1]?.id || null)}
                          onLeave={() => setHoveredCard(null)}
                          onClick={getCardClickHandler(currentCards[1])}
                          onNavigate={onNavigate}
                          className="w-full h-full"
                          isLoggedIn={isLoggedIn}
                          isEnrolledInFoundation={isEnrolledInFoundation}
                      ...
                      >
                        <GridCard
                          card={currentCards[2]}
                          isHovered={hoveredCard === currentCards[2]?.id}
                          onHover={() => setHoveredCard(currentCards[2]?.id || null)}
                          onLeave={() => setHoveredCard(null)}
                          onClick={getCardClickHandler(currentCards[2])}
                          onNavigate={onNavigate}
                          className="w-full h-full"
                          isLoggedIn={isLoggedIn}
                          isEnrolledInFoundation={isEnrolledInFoundation}
                      ...
                      >
                        <GridCard
                          card={currentCards[3]}
                          isHovered={hoveredCard === currentCards[3]?.id}
                          onHover={() => setHoveredCard(currentCards[3]?.id || null)}
                          onLeave={() => setHoveredCard(null)}
                          onClick={getCardClickHandler(currentCards[3])}
                          onNavigate={onNavigate}
                          className="w-full h-full"
                          isLoggedIn={isLoggedIn}
                          isEnrolledInFoundation={isEnrolledInFoundation}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-5 gap-2 md:gap-3 mb-4 md:mb-6 md:h-[400px]">
                      ...
                      <motion.div
                        key={currentCards[0]?.id}
                        variants={cardVariants}
                        className="md:col-span-3 h-[200px] md:h-full"
                      >
                        <GridCard
                          card={currentCards[0]}
                          isHovered={hoveredCard === currentCards[0]?.id}
                  ...
                  {isMobileView ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-6 gap-2 md:gap-2.5 mb-4 md:mb-6">
                      ...
                      <motion.div
                        key={currentCards[0]?.id}
                        variants={cardVariants}
                        className="md:col-span-6 h-[160px] md:h-[220px]"
                      >
                        <GridCard
                          card={currentCards[0]}
                          isHovered={hoveredCard === currentCards[0]?.id}
                ...
                <>
                  {isMobileView ? (
                    <div className="grid grid-cols-1 gap-2 mb-4">
                      ...
                      <motion.div
                        key={currentCards[4]?.id}
                        variants={cardVariants}
                        className="h-[220px]"
                      >
                        <GridCard
                          card={currentCards[4]}
                          isHovered={hoveredCard === currentCards[4]?.id}
                          onHover={() => setHoveredCard(currentCards[4]?.id || null)}
                          onLeave={() => setHoveredCard(null)}
                          onClick={getCardClickHandler(currentCards[4])}
                          onNavigate={onNavigate}
                          className="w-full h-full"
                          isLoggedIn={isLoggedIn}
                          isEnrolledInFoundation={isEnrolledInFoundation}
...
const GridCard: React.FC<GridCardProps> = ({
  ...
  const [isMobileViewCard, setIsMobileViewCard] = useState(false);
  const pauseTimeoutRef = useRef<NodeJS.Timeout | null>(null);

  // Detect mobile viewport for card rendering
  useEffect(() => {
    const checkMobileView = () => {
      setIsMobileViewCard(window.innerWidth < 768);
    };
  ...
  const displayTitle =
    isEnrolledInFoundation && card.enrolledTitle
      ? card.enrolledTitle
      : isLoggedIn && card.loggedInTitle
        ? card.loggedInTitle
        : card.title;
  const displaySubtitle =
    isEnrolledInFoundation && card.enrolledSubtitle
  ...
  const currentDynamicTitle =
    card.id === 'card-2'
      ? card.dynamicTitles
        ? card.dynamicTitles[currentImageIndex === 1 ? animationSceneIndex + 1 : currentImageIndex]
        : null
      : null;

  // Get current dynamic subtitle for discover card and card-2
  const currentDynamicSubtitle =
    card.id === 'card-2'
      ? card.dynamicSubtitles
        ? card.dynamicSubtitles[
            currentImageIndex === 1 ? animationSceneIndex + 1 : currentImageIndex
          ]
        : null
      : null;
  ...
  const displayTitleOverride =
    card.id === 'programs' ? 'Our Mission @ Pilotshortage.org' : finalDisplayTitle;
  const displaySubtitleOverride =
    card.id === 'programs'
      ? 'Find structured flight training tracks that launch you from school to airline-ready status with verified progress and career-focused pathways.'
  ...
  const renderMobileTitle = (title: string) => {
    ...
    if (card.id === 'pilot-recognition') {
      return (
        <>
          Recognition Profile{' '}
          <span className="text-red-600 underline underline-offset-2 decoration-[1px] decoration-red-600 hover:decoration-red-700 transition-all">
            Learn More
          </span>
        </>
      );
  ...
  const shouldUseEnrolledCarousel =
    isEnrolledInFoundation && card.isCarouselWhenEnrolled && card.enrolledImages;
  const shouldUseLoggedInCarousel =
    isLoggedIn && !isEnrolledInFoundation && card.isCarouselWhenLoggedIn && card.loggedInImages;
  ...
  const handleManualNavigation = (direction: 'next' | 'prev') => {
    ...
    // Navigate and reset animation scene
    if (direction === 'next') {
      setCurrentImageIndex((prev) => (prev + 1) % carouselImages.length);
    } else {
  ...
  return (
    ...
    >
      {/* MOBILE NEWS-STYLE CARD - Full-bleed image with overlay text */}
      {!forceDesktop && isMobileViewCard && (
        <div className="relative w-full h-full rounded-2xl overflow-hidden cursor-pointer group">
          ...
          <div className="absolute inset-x-0 bottom-0 p-5 flex items-center justify-between">
            <span className="inline-flex items-center gap-2 text-white uppercase font-bold text-xs tracking-[0.15em]">
              Learn More
              <svg
                className="w-4 h-4"
                fill="none"
            ...
            {card.badge && (
              <span className="px-2.5 py-1 bg-white/90 text-slate-900 text-[10px] font-bold uppercase tracking-wider rounded-sm">
                {card.badge}
              </span>
            )}
          </div>
        ...
        <>
          ...
          {card.isDirectory ? (
            ...
            >
              ...
              <div
                className={`absolute bottom-0 left-0 right-0 h-[3px] z-30 transition-opacity duration-300 ${isMsfsSelected ? 'opacity-100 bg-[#00b4d8]' : 'opacity-0 bg-transparent'}`}
              />
              ...
              <div className="absolute inset-0">
                {card.videoUrl ? (
                  ...
                  <div className="relative w-full h-full" onClick={(e) => e.stopPropagation()}>
                    ...
                    {!(card.isCarousel && carouselImages) &&
                      (card.videoUrl || card.image || card.hasAnimation) &&
                      !(isLoggedIn && card.hasAnimationWhenLoggedIn === false) && (
                  ...
                  <div className="relative w-full h-full">
                    {carouselImages.map((img, idx) => (
                      <div
                        key={idx}
                        className={`absolute inset-0 w-full h-full ${enableAnimations ? 'transition-all duration-1000' : ''} ${idx === currentImageIndex ? 'opacity-100 scale-100' : 'opacity-100 scale-105'}`}
                      >
                        <img
                          src={img}
                  ...
                  <div className="relative w-full h-full">
                    {carouselImages.map((img, idx) => {
                      ...
                      return (
                        <div
                          key={idx}
                          className={`absolute inset-0 w-full h-full ${enableAnimations ? 'transition-all duration-700 ease-out' : ''} ${idx === currentImageIndex ? 'opacity-100 scale-100' : 'opacity-0 scale-105'}`}
                        >
                          {isAnimationIndex ? (
                            idx === 1 && card.id === 'discover' ? (
                              <DiscoverPathwaysAnimation
                                isPlaying={idx === currentImageIndex}
                                onSceneChange={setAnimationSceneIndex}
                              />
                            ) : idx === 1 ? (
                              <EtihadExpectationsAnimation
                                isHovered={isHovered && idx === currentImageIndex}
                              />
                            ) : (
                              <DigitalLogbookAnimation
                                isHovered={isHovered && idx === currentImageIndex}
                              />
                            )
                          ) : (
                ...
                >
                  <div className="flex flex-col justify-between min-h-[68px] md:min-h-[74px] lg:min-h-[80px]">
                    ...
                    <div className="flex items-center gap-1.5 mb-0.5">
                      <span
                        className={`text-[10px] md:text-[11px] font-bold ${card.isLightMode ? 'text-slate-700' : isMsfsSelected ? 'text-white' : 'text-white/80'}`}
                      >
                        &#8811;
                      </span>
                      <h3
                        className={`text-sm font-bold uppercase tracking-[0.12em] ${card.isLightMode ? 'text-slate-900' : 'text-white'} ${card.id === 'credentials' ? 'text-black' : ''}`}
                      >
                        {card.id === 'pilot-recognition' ? (
                          <>
                            Recognition Profile{' '}
                    ...
                    <div
                      className={`w-full max-w-[80px] h-0.5 mb-1.5 ${card.isLightMode ? 'bg-gradient-to-r from-slate-400 to-transparent' : isMsfsSelected ? 'bg-gradient-to-r from-white to-transparent' : 'bg-gradient-to-r from-[#00b4d8] to-transparent'}`}
                    />
                    {/* Description - dark text for light mode, light text for dark */}
                    <p
                      className={`text-[9px] leading-snug line-clamp-2 ${card.isLightMode ? 'text-slate-600' : isMsfsSelected ? 'text-white/85' : 'text-slate-300'}`}
                    >
                      {finalDisplaySubtitle}
                    </p>
              ...
              {!isLargeCard && (
                <div className="absolute bottom-0 left-0 right-0">
                  ...
                  >
                    ...
                    <div
                      className={`absolute top-0 left-0 w-10 h-[2px] ${card.isLightMode ? 'bg-slate-400' : isMsfsSelected ? 'bg-white' : 'bg-[#00b4d8]'}`}
                    />
                    <div className="flex items-center gap-2">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1 mb-0.5">
                          <span
                            className={`text-[10px] md:text-xs font-bold ${card.isLightMode ? 'text-slate-700' : isMsfsSelected ? 'text-white' : 'text-white/80'}`}
                          >
                            &#8811;
                          </span>
                          <h3
                            className={`font-bold text-xs md:text-sm truncate uppercase tracking-wider ${card.isLightMode ? 'text-slate-900' : 'text-white'}`}
                          >
                            {displayTitle}
                          </h3>
                        </div>
                        <p
                          className={`text-[9px] md:text-[10px] line-clamp-3 leading-tight ${card.isLightMode ? 'text-slate-600' : isMsfsSelected ? 'text-white/85' : 'text-slate-300'}`}
                        >
                          {finalDisplaySubtitle}
                        </p>
                      </div>
