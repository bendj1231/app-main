  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const data = rows?.[0];
        if (data) setProfile({ total_hours: data.total_hours as number, licenses: data.licenses as string[], type_ratings: data.type_ratings as string[], logged_hours: data.logged_hours as number, program_inputs: data.program_inputs as any });
