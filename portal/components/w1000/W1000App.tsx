...
import { playSound, toggleMute, getMuted, startMusic } from './services/audioService';
...
const WelcomeScreen: React.FC<{ onStart: () => void }> = ({ onStart }) => {
    ...
    useEffect(() => {
        ...
        // 0.1s: Logo start pull-back
        // 0.4s: Fade In "Welcome"
        // 1.4s: Fade Out "Welcome"
        // 1.9s: Fade In "Fellow pilot."
        // 3.2s: Exit sequence
...
const App: React.FC<{ userProfile?: { displayName?: string; email?: string; avatarUrl?: string } }> = ({ userProfile }) => {
  ...
  // Handle internal URL routing for W1000 pages
  useEffect(() => {
  ...
  const SideControls = ({ mode }: { mode: InterfaceMode }) => (
    <>
      {withHint("Toggle Controls Panel", 
          ...
          className={`absolute ${showSidePanel ? 'left-24' : 'left-2'} top-1/2 -translate-y-1/2 z-[60] bg-[#1a1a1d] border-2 border-black/80 w-10 h-24 rounded-r-md shadow-2xl flex flex-col items-center justify-center gap-1 transition-all duration-500 group active:scale-95 overflow-hidden hover:border-g1000-cyan/30`}
          >
      ...
      <div className={`absolute left-2 top-1/2 -translate-y-1/2 flex flex-col gap-6 z-50 w-24 items-center pointer-events-auto py-5 bg-[#141414] rounded-r-xl border-y-2 border-r-2 border-black shadow-[0_20px_50px_rgba(0,0,0,1),5px_0_15px_rgba(0,0,0,0.5)] transition-all duration-500 ${showSidePanel ? 'translate-x-0' : '-translate-x-[120%]'}`}>
                ...
                <div key={activeModule} className="flex-1 overflow-hidden relative animate-app-pop-up">
                    {activeModule === Page.SIMULATOR ? (
                         <div className="absolute inset-0 bg-black">
                            <SimulatorRoom onExit={handleGoHome} />
                         </div>
                    ) : (
                         <div className="h-full w-full relative">
                             {renderMfdContent()}
