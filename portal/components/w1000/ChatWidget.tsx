...
const ChatWidget: React.FC<ChatWidgetProps> = ({ forceOpen = false }) => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] flex flex-col items-end transition-all duration-300">
      {isOpen && (
        <div className="mb-4 w-96 bg-slate-800 border border-slate-600 rounded-2xl shadow-2xl overflow-hidden flex flex-col h-[500px]">
          {/* Header */}
          ...
          <div className="p-4 bg-slate-800 border-t border-slate-700">
            <div className="relative">
              <input
                ...
                className="w-full bg-slate-800 border border-slate-600 rounded-xl py-3 pl-4 pr-12 text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all text-sm"
              />
              <button 
                onClick={handleSend}
      ...
      {!isOpen && (
        <button 
          ...
          className="group flex items-center gap-3 bg-blue-600 hover:bg-blue-500 text-white p-4 rounded-full shadow-xl shadow-blue-900/50 transition-all hover:scale-105 border-2 border-white"
        >
