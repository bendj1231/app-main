...
const Dashboard: React.FC<DashboardProps> = ({ setPage, onSelectSubPage, welcomeComplete = true }) => {
  ...
  return (
    <div className={`h-full w-full font-sans flex flex-col relative selection:bg-cyan-500/30 text-slate-800 overflow-hidden`}>
      ...
      {/* --- TOP APP BAR --- */}
      <div className={`h-20 flex items-center justify-between px-8 z-50 shrink-0 bg-gradient-to-b from-white/80 to-transparent relative transition-all duration-1000 ease-[cubic-bezier(0.25,0.8,0.25,1)] ${isLoaded && !isNavigatingAway ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0'}`}>
         <div className="absolute left-1/2 top-4 -translate-x-1/2 flex flex-col items-center">
            <h1 className="text-3xl md:text-4xl font-black font-sans uppercase tracking-[0.2em] text-transparent bg-clip-text bg-gradient-to-b from-slate-900 via-slate-700 to-slate-400 drop-shadow-sm leading-none">WING MENTOR</h1>
            <div className="text-sky-600 font-mono text-xs font-bold tracking-[0.5em] mt-1 uppercase">G1000 SYSTEM</div>
         </div>
         <div className="ml-auto flex items-center gap-6">
            <div className="hidden md:flex items-center gap-4 bg-white/40 backdrop-blur-md border border-white/40 px-4 py-2 rounded-full shadow-sm">
            ...
            <div className="relative">
                <Bell className="w-5 h-5 text-slate-600 hover:text-slate-900 transition-colors cursor-pointer" />
      ...
      <div className={`w-full flex flex-col items-center justify-center py-2 z-40 relative gap-2 transition-all duration-1000 delay-100 ease-[cubic-bezier(0.25,0.8,0.25,1)] ${isLoaded && !isNavigatingAway ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-90 -translate-y-4 blur-sm'}`}>
          <div className="group relative transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] w-80 h-10 hover:h-48 bg-white/60 hover:bg-white/95 backdrop-blur-md border border-slate-200 hover:border-slate-300 rounded-full hover:rounded-2xl shadow-lg hover:shadow-[0_20px_40px_rgba(0,0,0,0.1)] overflow-hidden cursor-pointer">
      ...
      <div className={`flex-1 relative flex flex-col items-center justify-center min-h-0 transition-all duration-1000 delay-200 ease-out ${isLoaded ? 'opacity-100 scale-100 blur-0' : 'opacity-0 scale-95 blur-md'}`}>
          <div className={`relative w-full max-w-5xl h-[280px] flex items-center justify-center z-10 mt-4 perspective-[1200px]`}>
              <button onClick={prevSlide} className={`absolute left-4 md:left-20 z-50 p-3 rounded-full bg-white/20 border border-white/40 hover:bg-white/40 transition-all group backdrop-blur-md shadow-lg ${isNavigatingAway ? 'opacity-0' : 'opacity-100'}`}><ChevronLeft className="w-6 h-6 text-slate-800" /></button>
              <button onClick={nextSlide} className={`absolute right-4 md:right-20 z-50 p-3 rounded-full bg-white/20 border border-white/40 hover:bg-white/40 transition-all group backdrop-blur-md shadow-lg ${isNavigatingAway ? 'opacity-0' : 'opacity-100'}`}><ChevronRight className="w-6 h-6 text-slate-800" /></button>
              ...
              {carouselData.map((item, index) => {
                  ...
                  const transform = `translateX(${offset * 60}%) scale(${isActive ? 1 : 0.85}) translateZ(${isActive ? 0 : -100}px) rotateY(${offset * -25}deg)`;
                  ...
                  return (
                      <div key={item.id} className={`absolute w-[260px] md:w-[400px] h-[200px] md:h-[260px] cursor-pointer transition-all duration-700`} style={{ transform, opacity, zIndex: 50 - absOffset, filter }} onClick={() => isActive ? handleCardClick(item) : setActiveIndex(index)}>
                         <div className="w-full h-full rounded-2xl overflow-hidden border border-white/50 bg-white/20 backdrop-blur-2xl shadow-2xl relative group">
                            ...
                            <div className={`absolute inset-0 flex flex-col ${isMasterclass ? 'justify-end items-center text-center p-6 pb-2' : 'justify-end p-6'}`}>
                                {!isMasterclass && item.subtitle && (
                                    <div className="mb-auto flex justify-between items-start">
                                        <div className="px-2 py-1 bg-white/20 border border-white/30 rounded backdrop-blur-md shadow-sm">
                                            <span className="text-[9px] font-black text-white uppercase tracking-widest drop-shadow-sm">{item.subtitle}</span>
                                        </div>
                                    </div>
                                )}
                                <h2 className={`font-black text-white tracking-tight drop-shadow-md ${isMasterclass ? 'text-[14px] mb-0 px-4 leading-[1.2] uppercase opacity-90 text-center' : 'text-2xl md:text-3xl mb-2'}`}>
                                  {isMasterclass ? (
          ...
          <div className={`h-24 w-full flex items-center justify-center mt-4 z-10 transition-all duration-300 ${isNavigatingAway ? 'opacity-0 translate-y-10 blur-md' : 'opacity-100'}`}>
              <div key={activeIndex} className="max-w-lg text-center px-6 animate-slide-up">
                  <h3 className="text-slate-500 text-xs font-black tracking-[0.3em] uppercase mb-2">Selected Topic</h3>
                  <p className="text-slate-800 text-sm md:text-base font-medium leading-relaxed">{carouselData[activeIndex]?.desc}</p>
              </div>
          </div>
      </div>

      {/* --- BOTTOM DOCK --- */}
      <div className={`h-64 shrink-0 flex flex-col items-center justify-start pt-2 pb-10 gap-2 transition-all duration-1000 delay-300 ease-[cubic-bezier(0.25,0.8,0.25,1)] ${isLoaded && !isNavigatingAway ? 'translate-y-0 opacity-100' : 'translate-y-40 opacity-0'}`}>
          <div className="flex items-end gap-6 px-4 py-1 relative">
              {dockItems.map((item) => (
                  <button key={item.id} onClick={() => handleDockNav(item)} className={`group relative flex flex-col items-center gap-2 px-2 py-1 transition-all duration-300 hover:-translate-y-4`}>
                      <div className="w-16 h-16 md:w-20 md:h-20 relative rounded-2xl overflow-hidden shadow-lg group-hover:shadow-xl transition-shadow">
                      ...
                      <div className="w-1.5 h-1.5 rounded-full bg-slate-300 group-hover:bg-cyan-500 transition-all duration-300 opacity-0 group-hover:opacity-100 translate-y-2"></div>
                  </button>
              ))}
          </div>
          <div className={`w-48 h-[1px] bg-gradient-to-r from-transparent via-slate-400/40 to-transparent shadow-[0_1px_0_rgba(255,255,255,0.2)]`}></div>
          <div className={`text-[9px] font-black uppercase tracking-[0.2em] font-mono flex gap-1.5 mt-1`}>
