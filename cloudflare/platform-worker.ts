...
async function getJWKS(env: Env): Promise<JWKSKey[]> {
  ...
  if (!jwksRes.ok)
    throw new Response(JSON.stringify({ error: 'Failed to fetch JWKS' }), { status: 500 });
...
async function verifyAuth0Token(request: Request, env: Env): Promise<JWTPayload> {
  ...
  if (parts.length !== 3)
    throw new Response(JSON.stringify({ error: 'Invalid JWT' }), { status: 401 });
  const headerJson = JSON.parse(atob(parts[0]));
  const kid = headerJson.kid;

  const keyData = jwksKeys.find((k) => k.kid === kid);
  if (!keyData)
    throw new Response(JSON.stringify({ error: 'Signing key not found' }), { status: 401 });
  ...
  if (payload.exp * 1000 < Date.now())
    throw new Response(JSON.stringify({ error: 'Token expired' }), { status: 401 });
...
async function getProfileByAuth0Id(
  db: D1Database,
  auth0Id: string
): Promise<Record<string, unknown> | null> {
  return db
    .prepare('SELECT * FROM profiles WHERE auth0_id = ?')
    .bind(auth0Id)
...
async function getProfileById(db: D1Database, id: string): Promise<Record<string, unknown> | null> {
  return db.prepare('SELECT * FROM profiles WHERE id = ?').bind(id).first() as Promise<Record<
    string,
    unknown
  > | null>;
}
...
async function ensureProfile(
  ...
  if (profile && !profile['public_token']) {
    const publicToken = generatePublicToken();
    await dbProfiles
      .prepare(`UPDATE profiles SET public_token = ? WHERE id = ?`)
      .bind(publicToken, profile['id'])
      .run();
...
async function handleRequest(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    if (err instanceof Response) {
      ...
      return new Response(body, {
        status: err.status,
        headers: { ...Object.fromEntries(err.headers.entries()), ...corsHeaders(origin) },
      });
    }
  ...
  if (path === '/api' && method === 'POST') {
    ...
    const profileActions = [
      ...
      'createCredential',
      'getCredentials',
      'getVerificationReceipts',
      'submitVerification',
      'getVerificationByAccountNumber',
    ...
    if (action === 'batch') {
      ...
      for (const req of requests) {
        try {
          results[req.action] = await handleAction(
            req.action,
            req.params || {},
            dbOps,
            dbProfiles,
  ...
  switch (action) {
    ...
    case 'upsertProfile': {
      ...
      if (!profile) {
        ...
        const countRow = (await dbProfiles
        ...
        await dbProfiles
          .prepare(`INSERT INTO profiles (${columns.join(', ')}) VALUES (${placeholders})`)
          .bind(...values)
          .run();
        profile = await getProfileById(dbProfiles, id);
      } else {
    ...
    case 'getRecognitionScore': {
      ...
      const row = await dbProfiles
        .prepare('SELECT * FROM recognition_scores WHERE user_id = ?')
        .bind(userId)
        .first();
    ...
    case 'saveRecognitionScore': {
      ...
      if (missing) throw new Error(missing);
      const userId = params.user_id as string;
      const existing = await dbProfiles
        .prepare('SELECT id FROM recognition_scores WHERE user_id = ?')
        .bind(userId)
        .first();
      if (existing) {
        await dbProfiles
    ...
    case 'getRecognitionPlusMember': {
      ...
      const member = await db
        .prepare('SELECT * FROM recognition_plus_members WHERE auth0_sub = ?')
        .bind(auth0Sub)
        .first();
    ...
    case 'createRecognitionPlusMember': {
      ...
      const existing = await db
        .prepare('SELECT id FROM recognition_plus_members WHERE auth0_sub = ?')
        .bind(auth.sub)
        .first();
      ...
      `
        ...
        .bind(id, userId, auth.sub, tier, 'active', 'pending', now, now)
        .run();
    ...
    case 'updateRecognitionPlusMember': {
      ...
      if (params.veremark_checks_total !== undefined) {
        updates.push('veremark_checks_total = ?');
        values.push(params.veremark_checks_total);
      }
      if (params.veremark_checks_used !== undefined) {
        updates.push('veremark_checks_used = ?');
        values.push(params.veremark_checks_used);
      }
      ...
      values.push(userId);
      await db
        .prepare(`UPDATE recognition_plus_members SET ${updates.join(', ')} WHERE user_id = ?`)
        .bind(...values)
        .run();
      return db
        .prepare('SELECT * FROM recognition_plus_members WHERE user_id = ?')
        .bind(userId)
        .first();
    }
    case 'useVeremarkCheck': {
      ...
      return {
        success: true,
        checks_used: await db
          .prepare(
            'SELECT veremark_checks_used, veremark_checks_total FROM recognition_plus_members WHERE auth0_sub = ?'
          )
          .bind(auth0Sub)
          .first(),
      };
    ...
    case 'getVerificationReceipts': {
      ...
      return (results || []).map((row: unknown) => ({
        ...
        flags: (row as Record<string, unknown>).flags
          ? JSON.parse((row as Record<string, unknown>).flags as string)
          : [],
      }));
    }
    ...
    case 'createEnterprise': {
      ...
      `
        ...
        .bind(
          id,
          params.auth0_id || null,
          params.company_name,
          params.airline_name || params.company_name || null,
          params.airline_iata_code || null,
          params.airline_logo_url || null,
          params.airline_website || null,
          params.company_description || null,
          params.industry || null,
          params.account_type || 'airline',
          params.contact_email || null,
          params.contact_phone || null,
          params.billing_email || null,
          params.website || null,
          params.country || null,
          JSON.stringify(params.base_locations || []),
          JSON.stringify(params.fleet_information || {}),
          JSON.stringify(params.contact_information || {}),
          params.employee_count || 0,
          params.is_active !== false,
          params.account_tier || 'basic',
          params.can_pull_verified_profiles || false,
          params.can_view_pilot_details || false,
          params.can_export_data || false,
          params.max_pathway_cards || 5,
          params.max_interest_views_per_month || 100,
          params.stripe_customer_id || null,
          params.tier_expires_at || null,
          params.subscription_status || 'trial',
          params.subscription_tier || 'basic',
          now,
          now
        )
        .run();
    ...
    case 'getEnterprises': {
      const { results } = await db
        .prepare('SELECT * FROM enterprise_profiles ORDER BY created_at DESC LIMIT 200')
        .all();
      return results || [];
    }
    case 'getEnterprise': {
      ...
      if (!id) throw new Error('Missing id');
      const row = await db
        .prepare('SELECT * FROM enterprise_profiles WHERE id = ?')
        .bind(id)
        .first();
      if (!row) throw new Error('Not found');
      return row;
    ...
    case 'getEnterpriseByAuth0': {
      ...
      const row = await db
        .prepare('SELECT * FROM enterprise_profiles WHERE auth0_id = ? AND is_active = 1')
        .bind(auth0Id)
        .first();
    ...
    case 'updateEnterprise': {
      ...
      const allowed = [
        ...
        'is_active',
        'account_tier',
        'can_pull_verified_profiles',
        'can_view_pilot_details',
        'can_export_data',
        'max_pathway_cards',
        'max_interest_views_per_month',
        'stripe_customer_id',
        'tier_expires_at',
        'subscription_status',
        'subscription_tier',
      ...
      updates.push('updated_at = datetime("now")');
      await db
        .prepare(`UPDATE enterprise_profiles SET ${updates.join(', ')} WHERE id = ?`)
        .bind(...binds, id)
        .run();
      return db.prepare('SELECT * FROM enterprise_profiles WHERE id = ?').bind(id).first();
    }
    ...
    case 'enterprisePull': {
      ...
      const recognitionPlusOnly =
        params.recognition_plus_only === true ||
        params.recognition_plus_only === 'true' ||
        params.recognition_plus_only === 1;
        ...
        .bind(
          ...
          JSON.stringify({
            filters: {
              ...
              recognition_plus_only: recognitionPlusOnly,
              min_score: minScore,
              rating,
    ...
    case 'createFlightSchool': {
      ...
      const referralCode =
        ...
        `FS${Math.random().toString(36).substring(2, 8).toUpperCase()}`;
      await db
        .prepare(
          `
      ...
      `
        ...
        .bind(
          ...
          params.name,
          referralCode,
          params.commission_rate || 20,
          params.payout_method || 'bank_transfer',
          params.contact_email || null,
          params.contact_phone || null,
          params.country || null,
          params.website || null,
    ...
    case 'getFlightSchool': {
      ...
      const row = await db
        .prepare('SELECT * FROM flight_schools WHERE enterprise_id = ? AND is_active = 1')
        .bind(enterpriseId)
        .first();
    ...
    case 'getFlightSchoolReferrals': {
      ...
      const { results } = await db
        .prepare(
          'SELECT * FROM flight_school_referrals WHERE flight_school_id = ? ORDER BY created_at DESC LIMIT 200'
        )
        .bind(flightSchoolId)
        .all();
      return results || [];
    ...
    case 'getFlightSchoolPayouts': {
      ...
      const { results } = await db
        .prepare(
          'SELECT * FROM flight_school_payouts WHERE flight_school_id = ? ORDER BY created_at DESC LIMIT 100'
        )
        .bind(flightSchoolId)
        .all();
      return results || [];
    ...
    case 'getFlightSchoolNotifications': {
      ...
      const { results } = await db
        .prepare(
          'SELECT * FROM flight_school_notifications WHERE flight_school_id = ? ORDER BY created_at DESC LIMIT 50'
        )
        .bind(flightSchoolId)
        .all();
      return results || [];
    ...
    case 'createFlightSchoolReferral': {
      ...
      if (!flightSchoolId || !pilotEmail)
        throw new Error('Missing flight_school_id or pilot_email');
      const id = crypto.randomUUID();
      const now = new Date().toISOString();
      const school = (await db
        .prepare('SELECT referral_code, commission_rate FROM flight_schools WHERE id = ?')
        .bind(flightSchoolId)
    ...
    case 'markFlightSchoolNotificationRead': {
      const id = params.id as string;
      if (!id) throw new Error('Missing id');
      await db
        .prepare('UPDATE flight_school_notifications SET read = 1 WHERE id = ?')
        .bind(id)
        .run();
      return { success: true };
    }
    ...
    case 'getCredits': {
      ...
      if (!enterpriseId) throw new Error('Missing enterprise_id');
      const row = await db
        .prepare('SELECT * FROM enterprise_credits WHERE enterprise_id = ?')
        .bind(enterpriseId)
        .first();
    ...
    case 'getCreditTransactions': {
      ...
      const { results } = await db
        .prepare(
          'SELECT * FROM credit_transactions WHERE enterprise_id = ? ORDER BY created_at DESC LIMIT ?'
        )
        .bind(enterpriseId, limit)
        .all();
      return results || [];
    }
    case 'burnCredit': {
      ...
      const creditRow = (await db
        .prepare('SELECT balance FROM enterprise_credits WHERE enterprise_id = ?')
        .bind(enterpriseId)
      ...
      await db
        .prepare(
          'UPDATE enterprise_credits SET balance = ?, total_burned = total_burned + ?, updated_at = ? WHERE enterprise_id = ?'
        )
        .bind(newBalance, amount, now, enterpriseId)
        .run();
      await db
        .prepare(
          'INSERT INTO credit_transactions (id, enterprise_id, type, amount, balance_after, description, verification_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
        )
        .bind(txId, enterpriseId, 'burn', -amount, newBalance, description, verificationId, now)
        .run();
    ...
    case 'topUpCredits': {
      ...
      `
        ...
        .bind(crypto.randomUUID(), enterpriseId, amount, amount, now)
        .run();
      ...
      await db
        .prepare(
          'INSERT INTO credit_transactions (id, enterprise_id, type, amount, balance_after, description, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)'
        )
        .bind(txId, enterpriseId, 'top_up', amount, row.balance, `Top-up of ${amount} credits`, now)
        .run();
    ...
    case 'createSubscription': {
      ...
      `
        ...
        .bind(
          ...
          params.provider_subscription_id || null,
          now.toISOString(),
          periodEnd.toISOString(),
          params.cancel_at_period_end || 0,
          params.trial_end || null,
      ...
      const profileTable =
        params.subscriber_type === 'enterprise' ? 'enterprise_profiles' : 'profiles';
      await db
        .prepare(
          `UPDATE ${profileTable} SET subscription_tier = ?, subscription_status = ?, subscription_start_date = ?, subscription_end_date = ?, updated_at = ? WHERE id = ?`
        )
        .bind(
          params.tier,
          params.status || 'active',
    ...
    case 'getSubscription': {
      ...
      if (!subscriberType || !subscriberId)
        throw new Error('Missing subscriber_type or subscriber_id');
      const row = await db
        .prepare(
          'SELECT * FROM subscriptions WHERE subscriber_type = ? AND subscriber_id = ? AND status IN ("active","trial","past_due") ORDER BY current_period_end DESC LIMIT 1'
        )
        .bind(subscriberType, subscriberId)
        .first();
      return row || null;
    }
    case 'renewSubscription': {
      ...
      if (sub.cancel_at_period_end === 1)
        throw new Error('Subscription set to cancel, cannot renew');
      ...
      await db
        .prepare(
          'UPDATE subscriptions SET current_period_end = ?, renewal_count = ?, status = "active", updated_at = ? WHERE id = ?'
        )
        .bind(newEnd.toISOString(), renewalCount, now.toISOString(), id)
        .run();

      // Sync to subscriber profile
      const profileTable =
        sub.subscriber_type === 'enterprise' ? 'enterprise_profiles' : 'profiles';
      await db
        .prepare(
          `UPDATE ${profileTable} SET subscription_status = "active", subscription_end_date = ?, updated_at = ? WHERE id = ?`
        )
        .bind(newEnd.toISOString(), now.toISOString(), sub.subscriber_id)
        .run();
    ...
    case 'cancelSubscription': {
      ...
      if (sub.subscriber_type === 'enterprise') {
        await db
          .prepare(
            'UPDATE enterprise_profiles SET subscription_status = "cancelled", updated_at = ? WHERE id = ?'
          )
          .bind(now, sub.subscriber_id)
          .run();
      } else {
        await dbProfiles
          .prepare(
            'UPDATE profiles SET subscription_status = "cancelled", updated_at = ? WHERE id = ?'
          )
          .bind(now, sub.subscriber_id)
          .run();
      }
    ...
    case 'checkExpiredSubscriptions': {
      ...
      const { results } = await db
        .prepare(
          'SELECT * FROM subscriptions WHERE current_period_end < ? AND status IN ("active","trial","past_due") AND cancel_at_period_end = 0'
        )
        .bind(now)
        .all();
      ...
      for (const sub of expired) {
        ...
        if (sub.subscriber_type === 'enterprise') {
          await db
            .prepare(
              'UPDATE enterprise_profiles SET subscription_status = "expired", subscription_tier = "free", updated_at = ? WHERE id = ?'
            )
            .bind(now, sub.subscriber_id)
            .run();
        } else {
          await dbProfiles
            .prepare(
              'UPDATE profiles SET subscription_status = "expired", subscription_tier = "free", updated_at = ? WHERE id = ?'
            )
            .bind(now, sub.subscriber_id)
            .run();
        }
    ...
    case 'getLeaderboard': {
      ...
      binds.push(limit);
      const { results } = await dbProfiles
        .prepare(sql)
        .bind(...binds)
        .all();
      return results || [];
    ...
    case 'getUserRank': {
      const userId = params.user_id as string;
      if (!userId) throw new Error('Missing user_id');
      const userScore = (await dbProfiles
        .prepare('SELECT total_score FROM recognition_scores WHERE user_id = ?')
        .bind(userId)
        .first()) as { total_score: number } | null;
      if (!userScore) return 0;
      const { results } = await dbProfiles
        .prepare('SELECT COUNT(*) as count FROM recognition_scores WHERE total_score > ?')
        .bind(userScore.total_score)
        .all();
    ...
    case 'getScoreStatistics': {
      const userId = params.user_id as string;
      if (!userId) throw new Error('Missing user_id');
      const { results: allScores } = await dbProfiles
        .prepare('SELECT total_score FROM recognition_scores')
        .all();
      const scores = (allScores || []).map(
        (r) => (r as Record<string, unknown>)['total_score'] as number
      );
      const userRow = (await dbProfiles
        .prepare('SELECT total_score FROM recognition_scores WHERE user_id = ?')
        .bind(userId)
        .first()) as { total_score: number } | null;
    ...
    case 'getScoreHistory': {
      ...
      if (scoreType) {
        sql += ' AND score_type = ?';
        binds.push(scoreType);
      }
      if (days) {
        sql += " AND calculated_at >= datetime('now', '-" + days + " days')";
      }
    ...
    case 'logScore': {
      ...
      `
        ...
        .bind(id, userId, scoreType, scoreValue)
        .run();
      return { id };
    }
    ...
    case 'markScoreNotificationRead': {
      const id = params.id as string;
      if (!id) throw new Error('Missing id');
      await db.prepare('UPDATE score_notifications SET read = 1 WHERE id = ?').bind(id).run();
      return { updated: true };
    }
    case 'markAllScoreNotificationsRead': {
      const userId = params.user_id as string;
      if (!userId) throw new Error('Missing user_id');
      await db
        .prepare('UPDATE score_notifications SET read = 1 WHERE user_id = ? AND read = 0')
        .bind(userId)
        .run();
      return { updated: true };
    }
    ...
    case 'getCompletionTracking': {
      ...
      if (!userId) throw new Error('Missing user_id');
      const { results } = await db
        .prepare('SELECT * FROM completion_tracking WHERE user_id = ? ORDER BY updated_at DESC')
        .bind(userId)
        .all();
      return results || [];
    }
    case 'createCompletionTracking': {
      ...
      `
        ...
        .bind(
          ...
          params.item_type,
          params.item_title,
          params.completion_percentage || 0,
          params.status || 'in_progress',
          params.time_spent_hours || 0,
          params.score || null
        )
        .run();
    ...
    case 'updateCompletionTracking': {
      ...
      values.push(id);
      await db
        .prepare(`UPDATE completion_tracking SET ${sets.join(', ')} WHERE id = ?`)
        .bind(...values)
        .run();
      return db.prepare('SELECT * FROM completion_tracking WHERE id = ?').bind(id).first();
    }
    ...
    case 'getGoalTracking': {
      ...
      if (!userId) throw new Error('Missing user_id');
      const { results } = await db
        .prepare('SELECT * FROM goal_tracking WHERE user_id = ? ORDER BY created_at DESC')
        .bind(userId)
        .all();
      return results || [];
    }
    ...
    case 'updateGoal': {
      ...
      values.push(id);
      await db
        .prepare(`UPDATE goal_tracking SET ${sets.join(', ')} WHERE id = ?`)
        .bind(...values)
        .run();
      return db.prepare('SELECT * FROM goal_tracking WHERE id = ?').bind(id).first();
    }
    ...
    case 'getCoachingActions': {
      ...
      const { results } = await db
        .prepare('SELECT * FROM ai_coaching_actions WHERE pilot_id = ? ORDER BY created_at DESC')
        .bind(pilotId)
        .all();
      return results || [];
    }
    case 'createCoachingAction': {
      ...
      `
        ...
        .bind(id, params.pilot_id, params.action_type, params.description, params.score_boost || 0)
        .run();
    ...
    case 'queryTable': {
      ...
      const allowedTables = new Set([
        ...
        'learning_hours',
        'pilot_documents',
        'pilot_platform_connections',
        'logbook_hour_tokens',
        'p12_verification_events',
        'efb_complexity_tokens',
        'sim_session_tokens',
        'mfa_secrets',
        'referral_conversions',
        'referral_partners',
        'recognition_plus_referrals',
        'user_app_access',
        'job_opportunities',
        'enterprise_pathway_cards',
        'pathway_card_interests',
        'airline_expectations',
    ...
    case 'getAllPilots': {
      ...
      if (!me || me['role'] !== 'super_admin') throw new Error('Forbidden');
      const { results } = await dbProfiles
        .prepare('SELECT * FROM profiles ORDER BY created_at DESC LIMIT 500')
        .all();
      return results || [];
    }
    case 'updateUserTier': {
    ...
    case 'deleteAccount': {
      ...
      await dbProfiles
        .prepare('DELETE FROM recognition_scores WHERE user_id = ?')
        .bind(userId)
        .run();
    ...
    case 'issuerSign': {
      ...
      return {
        success: false,
        error: 'issuerSign not yet migrated to Worker. Use legacy issuer endpoint.',
      };
    }
    ...
    case 'createVoucherBatch': {
      ...
      `
        ...
        .bind(
          ...
          params.enterprise_id,
          params.batch_name,
          params.tier || 'recognition_plus',
          params.amount_cents || 2900,
          qty,
          0,
          0,
    ...
    case 'purchaseVoucherBatch': {
      ...
      const batch = (await db
        .prepare('SELECT * FROM bulk_voucher_batches WHERE id = ?')
        .bind(batchId)
        .first()) as Record<string, unknown> | null;
      ...
      const enterprise = (await db
        .prepare('SELECT company_name, contact_email FROM enterprise_profiles WHERE id = ?')
        .bind(batch.enterprise_id)
        .first()) as Record<string, unknown> | null;
      ...
      const dodoRes = await fetch('https://live.dodopayments.com/v1/checkouts', {
        ...
        body: JSON.stringify({
          product_cart: [
            {
              product_id: env.DODO_PRODUCT_ID_RECOGNITION_PLUS || 'voucher_batch',
      ...
      if (dodoData.session_id) {
        await db
          .prepare('UPDATE bulk_voucher_batches SET dodo_checkout_id = ? WHERE id = ?')
          .bind(dodoData.session_id, batchId)
          .run();
      }
      return {
    ...
    case 'generateVoucherCodes': {
      ...
      for (let i = 0; i < qty; i++) {
        ...
        await db
          .prepare(
            'INSERT INTO bulk_voucher_codes (id, batch_id, code, status, created_at) VALUES (?, ?, ?, ?, ?)'
          )
          .bind(crypto.randomUUID(), batchId, code, 'unused', now)
          .run();
      }

      await db
        .prepare('UPDATE bulk_voucher_batches SET codes_generated = ? WHERE id = ?')
        .bind(qty, batchId)
        .run();
    ...
    case 'getVoucherBatches': {
      ...
      const { results } = await db
        .prepare(
          'SELECT * FROM bulk_voucher_batches WHERE enterprise_id = ? ORDER BY created_at DESC LIMIT 50'
        )
        .bind(enterpriseId)
        .all();
      return results || [];
    ...
    case 'getVoucherCodes': {
      ...
      if (!batchId) throw new Error('Missing batch_id');
      const { results } = await db
        .prepare('SELECT * FROM bulk_voucher_codes WHERE batch_id = ? ORDER BY created_at DESC')
        .bind(batchId)
        .all();
      return results || [];
    }
    case 'redeemVoucher': {
      ...
      const voucher = (await db
        .prepare('SELECT * FROM bulk_voucher_codes WHERE code = ?')
        .bind(code)
        .first()) as Record<string, unknown> | null;
      ...
      const batch = (await db
        .prepare('SELECT * FROM bulk_voucher_batches WHERE id = ?')
        .bind(voucher.batch_id)
        .first()) as Record<string, unknown> | null;
      if (!batch) throw new Error('Batch not found');
      if (batch.status !== 'active') throw new Error('Batch not active');
      if (batch.expires_at && new Date(batch.expires_at as string) < new Date())
      ...
      await db
        .prepare(
          'UPDATE bulk_voucher_codes SET status = "redeemed", redeemed_by = ?, redeemed_at = ? WHERE id = ?'
        )
        .bind(pilotId, now, voucher.id)
        .run();
      // Update batch stats
      await db
        .prepare('UPDATE bulk_voucher_batches SET codes_redeemed = codes_redeemed + 1 WHERE id = ?')
        .bind(voucher.batch_id)
        .run();
      // Grant tier to pilot
      ...
      await dbProfiles
        .prepare(
          'UPDATE profiles SET subscription_tier = ?, subscription_status = "active", updated_at = ? WHERE id = ?'
        )
        .bind(tier, now, pilotId)
        .run();
    ...
    case 'revokeVoucher': {
      ...
      if (!id) throw new Error('Missing id');
      await db
        .prepare('UPDATE bulk_voucher_codes SET status = "revoked" WHERE id = ?')
        .bind(id)
        .run();
      return { success: true };
    }
    ...
    case 'triggerReferralPayout': {
      ...
      const partner = (await db
        .prepare('SELECT * FROM referral_partners WHERE id = ?')
        .bind(partnerId)
        .first()) as Record<string, unknown> | null;
      ...
      `
        ...
        .bind(payoutId, partnerId, payoutAmount, 'completed', partner.email || 'bank_transfer', now)
        .run();
      ...
      `
        ...
        .bind(payoutAmount, now, partnerId)
        .run();

      // Mark all pending conversions as paid
      await db
        .prepare(
          'UPDATE referral_conversions SET commission_status = "paid", paid_at = ? WHERE partner_id = ? AND commission_status = "pending"'
        )
        .bind(now, partnerId)
        .run();
    ...
    case 'getReferralPartners': {
      const { results } = await db
        .prepare('SELECT * FROM referral_partners ORDER BY total_referrals DESC LIMIT 200')
        .all();
      return results || [];
    }
    ...
    // ── Flight Crew Mentions / Verified Crew Tagging ──
    case 'tagFlightCrewMember': {
      const reporterUserId = params.reporter_user_id as string;
      const taggedUserId = params.tagged_user_id as string;
      const flight = params.flight as Record<string, unknown> | undefined;
      const role = params.role as string;
      const comment = params.comment as string | undefined;
      const reporterName = params.reporter_name as string | undefined;

      if (!reporterUserId || !taggedUserId || !flight || !role) {
        throw new Error('reporter_user_id, tagged_user_id, flight, and role are required');
      }

      await db
        .prepare(
          `
        CREATE TABLE IF NOT EXISTS flight_crew_mentions (
          id TEXT PRIMARY KEY,
      ...
      await db
        .prepare(
          `
        INSERT INTO flight_crew_mentions (
          id, reporter_user_id, tagged_user_id, flight_date, aircraft_type, registration, route, role, comment, status, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `
        )
        .bind(
      ...
      const flightDesc =
        [flight.aircraft_type, flight.date, flight.route].filter(Boolean).join(' · ') || 'a flight';
      const title = `${reporterName || 'Someone'} tagged you as crew`;
      const message = `They reported you flew ${flightDesc} acting as ${role}. Accept to add it to your record, or report it as falsified.`;
      ...
      return { success: true, mention_id: mentionId, notification_id: notificationId };
    }

    case 'respondToFlightCrewTag': {
      const mentionId = params.mention_id as string;
      const response = params.response as string;
      const userId = params.user_id as string;
      const reason = params.reason as string | undefined;

      if (!mentionId || !response || !userId) {
        throw new Error('mention_id, response, and user_id are required');
      }
      if (!['accept', 'report_falsified'].includes(response)) {
        throw new Error('response must be accept or report_falsified');
      }

      const status = response === 'accept' ? 'accepted' : 'reported_falsified';
      const now = new Date().toISOString();

      await db
        .prepare(
          `
        UPDATE flight_crew_mentions
        SET status = ?, reported_reason = ?, updated_at = ?
        WHERE id = ? AND tagged_user_id = ?
      `
        )
        .bind(status, reason || null, now, mentionId, userId)
        .run();

      return { success: true, status };
    }

    case 'getFlightCrewMentions': {
      const userId = params.user_id as string;
      const status = params.status as string | undefined;
      const limit = Math.min((params.limit as number) || 50, 100);
      if (!userId) throw new Error('Missing user_id');

      let sql = 'SELECT * FROM flight_crew_mentions WHERE tagged_user_id = ?';
      const binds: unknown[] = [userId];
      if (status) {
        sql += ' AND status = ?';
        binds.push(status);
    ...
    case 'getFlightHours': {
      ...
      return {
        ...
        disclaimer:
          'These hours are self-reported claims for pathway-matching only. They are NOT a regulatory logbook and cannot be used for licensing or certification.',
      };
    }
    ...
    case 'importFlightLogCSV': {
      ...
      `
        ...
        .bind(
          batchId,
          pilotId,
          (params.batch_name as string) || 'CSV Import',
          'csv_import',
          entryCount,
          dateFrom,
          dateTo,
          ...
          simH,
          'self_reported',
          (params.file_name as string) || 'import.csv',
          now
        )
        .run();
    ...
    case 'requestBatchVerification': {
      ...
      const batch = (await dbProfiles
        .prepare('SELECT id, pilot_profile_id, status FROM pilot_flight_log_batches WHERE id = ?')
        .bind(batchId)
        .first()) as Record<string, unknown> | null;
      if (!batch) throw new Error('Batch not found');
      ...
      await dbProfiles
        .prepare('UPDATE pilot_flight_log_batches SET status = ? WHERE id = ?')
        .bind('pending_verification', batchId)
        .run();
    ...
    case 'adminVerifyFlightLogBatch': {
      ...
      const batch = (await dbProfiles
        .prepare('SELECT * FROM pilot_flight_log_batches WHERE id = ? AND status = ?')
        .bind(batchId, 'pending_verification')
        .first()) as Record<string, unknown> | null;
      ...
      `
        ...
        .bind(
          ...
          batch['pic_hours'] || 0,
          batch['night_hours'] || 0,
          batch['instrument_hours'] || 0,
          batch['cross_country_hours'] || 0,
          batch['multi_engine_hours'] || 0,
          batch['jet_hours'] || 0,
          batch['turbine_hours'] || 0,
          batch['simulator_hours'] || 0,
          'recognition_plus_verified',
          verifiedBy || 'admin',
          now,
          now,
          now
      ...
      `
        ...
        .bind('verified', verifiedBy || 'admin', verificationNotes, now, traceId, batchId)
        .run();
      ...
      `
        ...
        .bind(
          ...
          verifiedBy || 'admin',
          'recognition_plus_admin',
          'pending_verification',
          'verified',
          signedDocumentUrl || null,
          JSON.stringify({
            cfi_name: cfiName,
            cfi_license: cfiLicense,
            ato_name: atoName,
            batch_id: batchId,
    ...
    case 'getPendingVerificationBatches': {
      ...
      `
        ...
        .bind('pending_verification', limit, offset)
        .all();
      return results || [];
    }
    ...
    case 'rejectFlightLogBatch': {
      ...
      await dbProfiles
        ...
        .bind('rejected', rejectionReason, now, batchId)
        .run();
      ...
      `
        ...
        .bind(
          crypto.randomUUID(),
          batchId,
          batch['pilot_profile_id'] as string,
    ...
    case 'deleteFlightLogBatch': {
      ...
      await dbProfiles
        .prepare('DELETE FROM pilot_flight_log_batches WHERE id = ?')
        .bind(batchId)
        .run();
  ...
  if (body.event_type === 'payment.succeeded' || body.status === 'completed') {
    ...
    if (voucherType === 'voucher_batch' && batchId) {
      try {
        ...
        // Mark batch as paid
        await env.DB_OPS.prepare(
          'UPDATE bulk_voucher_batches SET payment_status = "paid", updated_at = ? WHERE id = ?'
        )
          .bind(now, batchId)
          .run();
        ...
        // Auto-generate codes
        const batch = (await env.DB_OPS.prepare('SELECT * FROM bulk_voucher_batches WHERE id = ?')
          .bind(batchId)
          .first()) as Record<string, unknown> | null;
        if (batch && batch.codes_generated === 0) {
          ...
          await env.DB_OPS.prepare(
            'UPDATE bulk_voucher_batches SET codes_generated = ? WHERE id = ?'
          )
            .bind(qty, batchId)
            .run();
        }
...
async function handleCheckout(request: Request, env: Env): Promise<Response> {
  ...
  const successUrl =
    ...
    'https://pilotrecognition.com/platform?tab=verification&checkout=success';
  ...
  try {
    ...
    if (!checkoutRes.ok) {
      ...
      const checkoutError = checkoutData as { message?: string };
      return jsonResponse(
        { error: checkoutError.message || 'Checkout creation failed' },
        checkoutRes.status,
        origin
      );
    }
    ...
    const errMessage = err instanceof Error ? err.message : 'Checkout creation failed';
    console.error('[Dodo Checkout] exception:', errMessage);
    return jsonResponse({ error: errMessage }, 500, origin);
  }
}
