...
const JURISDICTION_MAP: Record<string, JurisdictionInfo> = {
    ...
    MU: { country: 'the Republic of Mauritius', countryCode: 'MU', governingLaw: 'the laws of the Republic of Mauritius, including the Data Protection Act 2017 and the Electronic Transactions Act 2000', dataAuthority: 'Data Protection Office, Republic of Mauritius', electronicConsentLaw: 'Electronic Transactions Act 2000', aviationAuthority: 'Department of Civil Aviation (DCA Mauritius)', privacyFramework: 'Data Protection Act 2017' },
};

const DEFAULT_JURISDICTION: JurisdictionInfo = {
    country: 'the Republic of Mauritius',
    countryCode: 'MU',
    governingLaw: 'the laws of the Republic of Mauritius, including the Data Protection Act 2017 and the Electronic Transactions Act 2000',
    dataAuthority: 'Data Protection Office, Republic of Mauritius',
    electronicConsentLaw: 'Electronic Transactions Act 2000',
    aviationAuthority: 'your regional Civil Aviation Authority (CAA)',
    privacyFramework: 'Mauritius Data Protection Act 2017 (primary) / applicable regional data protection regulations',
};
    ...
    return (
        <div className="min-h-screen bg-white text-slate-900 font-sans">
            ...
            <div className="pt-32 pb-12 px-6 max-w-4xl mx-auto">
                ...
                <p className="text-sm text-slate-500 mb-1">Governing Jurisdiction: Republic of Mauritius</p>
                <p className="text-sm text-slate-500 mb-1">Statutory Baseline: Data Protection Act 2017 · Electronic Transactions Act 2000</p>

                <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 mb-8">
                    <p className="text-xs text-amber-800 leading-relaxed">
                        <strong>Entity Disclosure:</strong> PilotRecognition.com is currently operated by <strong>Benjamin Bowler</strong> as an individual entrepreneur pending the incorporation of <strong>Aviation Pathways Ltd</strong> in the Republic of Mauritius. All contractual obligations, data controller responsibilities, and liabilities described in this Agreement are presently vested in Benjamin Bowler and shall be novated to Aviation Pathways Ltd upon issuance of the Certificate of Incorporation by the Companies and Business Registration Division (CBRD). Until such novation occurs, Benjamin Bowler is the legal Data Controller and Platform Operator. All references to "Aviation Pathways Ltd" in this Agreement shall be read as referring to Benjamin Bowler in his capacity as individual entrepreneur pending such novation.
                    </p>
                </div>
                {!loading && (
                ...
                <div className="space-y-8 text-slate-700">
                    ...
                    <section>
                        <div className="bg-slate-900 text-white rounded-xl p-5 mb-6">
                            ...
                            <p className="font-bold text-base mb-1">Data Controller · SaaS Infrastructure Provider · Digital Credential Record Manager</p>
                            <p className="text-slate-300 text-xs leading-relaxed mb-2">Aviation Pathways Ltd operates the platform infrastructure, manages user accounts, and maintains digitally signed verification records as digital access tickets. User profiles, credential metadata, and signed verification records are stored in encrypted database environments (AES-256-GCM at rest). Raw documents uploaded for verification are forwarded to third-party verifiers and not retained.</p>
                            <p className="text-slate-400 text-xs"><span className="text-slate-300 font-semibold">DATA FLOW:</span> The Platform receives minimal structured verification outcomes (status, check ID, timestamp) from independent third-party verifiers and maintains signed verification records. Detailed verification receipts containing full credential data are sent directly to the User's email by the verifier and are never transmitted to, viewed by, or stored on the Platform.</p>
                        </div>
                        ...
                        <p className="mb-3 text-sm"><strong>Third-Party Verification:</strong> The Platform Operator routes verification requests to independent third-party verification partners (e.g., Veremark Ltd.) who verify credentials against civil aviation authorities. The Platform receives only minimal structured outcomes from these partners and maintains digitally signed verification records as access tickets. Detailed verification receipts are sent directly to the User's email by the verifier.</p>
                        <p className="mb-3 text-sm"><strong>Infrastructure Scope:</strong> The Platform provides software infrastructure including database storage (Supabase, Neon PostgreSQL, MongoDB), authentication (Auth0), payment processing (Stripe), and API routing. The Platform Operator's duty of care covers technical availability, data security, and compliance with applicable data protection laws.</p>
                        <p className="mb-4 text-sm hidden deleted">“Infrastructure” is restricted to the software conduits, multi-engine distributed database synchronization layers, and application programming interface (API) gateways facilitating the flow of tokenized metadata between the User and independent third-party integration networks. The Platform Operator’s operational and statutory duty of care is limited exclusively to the technical availability and structural integrity of this infrastructure layer.</p>
                    ...
                    <section>
                        ...
                        <h3 className="font-semibold text-slate-800 mb-2">1.1 User Account &amp; Data Controller Relationship</h3>
                        <p className="mb-3 text-sm">Upon onboarding, the User acknowledges that Aviation Pathways Ltd is the <strong>Data Controller</strong> for all personal data processed through the Platform infrastructure. The User retains ownership of their identity credentials and controls what data to share, with whom, and when. The Platform Operator manages the infrastructure, stores encrypted profile data and signed verification records, and routes verification requests to third-party providers.</p>
                        <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 mb-4">
                            <p className="text-blue-800 text-xs font-bold uppercase tracking-wide mb-1">Secure Digital Profile & Server-Side Records</p>
                            <p className="text-blue-900 text-xs leading-relaxed mb-2">The platform maintains digitally signed <strong>verification records</strong> under <code>did:web:pilotrecognition.com</code> based on minimal structured verification outcomes received from third-party verifiers. These records are delivered to the User’s personal Secure Profile and retained server-side as signed records for pathway gating and revocation. The User can export their records to any compliant digital identity framework.</p>
                            <p className="text-blue-800 text-xs font-bold uppercase tracking-wide mb-1 mt-2">External Digital Identity Syncing</p>
                            <p className="text-blue-900 text-xs leading-relaxed mb-3">The User may synchronise a pre-existing, compliant external digital identity profile. During an external sync, the Platform infrastructure reads only the public identifier descriptor string — private keys remain under the User’s control. Any historical credential claims from an externally synced profile are flagged as <strong>“Pending Verification”</strong> until processed through the platform’s independent verification ecosystem.</p>
                            <p className="text-blue-800 text-xs font-bold uppercase tracking-wide mb-1">Data Integrity & Verification</p>
                            <p className="text-blue-900 text-xs leading-relaxed">The User acknowledges that modifications to profile or flight hours within the database layer are <strong>not valid for verification or operator pathway matching</strong> until processed through the platform’s independent verification ecosystem. The Platform Operator holds no liability for mismatched data states caused by unverified user inputs.</p>
                        </div>

                        <p className="mb-4 text-sm">The Platform Operator’s distributed database caching infrastructure stores user profile data, credential metadata, verification outcomes, and signed verification records in encrypted database environments (AES-256-GCM at rest via Supabase). The Platform acts as the Data Controller for this personal data under the Mauritius Data Protection Act 2017. Raw documents uploaded for verification are forwarded to third-party verifiers and are not retained by the Platform.</p>

                        <h3 className="font-semibold text-slate-800 mb-2">1.2 Authentication &amp; Passkey Security</h3>
                        <p className="mb-3 text-sm">Passkey authentication uses device-bound cryptographic keys held exclusively by the User. The Platform never stores or accesses private keys:</p>
                            ...
                            <table className="w-full text-sm border-collapse">
                                <tbody>
                                    {[
                                        ...
                                        ['Passkey Private Key Custody', 'Private cryptographic keys for passkey authentication are stored exclusively on the User\'s device hardware (Secure Enclave / TPM) and synced encrypted by Google Password Manager or iCloud Keychain. The Platform stores only the corresponding public key for signature verification.'],
                                        ['Irrecoverable Loss Acknowledgement', 'Loss of access to your device-level passkey architecture may result in inability to authenticate. Account recovery requires contacting the Platform with identity verification.'],
                                    ].map(([label, desc], i) => (
                    ...
                    <section>
                        ...
                        <div className="border-l-4 border-emerald-500 bg-emerald-50 rounded-r-xl px-5 py-3 mb-3">
                            ...
                            <p className="text-emerald-900 text-xs leading-relaxed">Transition between Terminal 2 and Terminal 3 is not user-selectable. It is triggered exclusively by the successful completion of an independent secure API handshake with a registered civil aviation verification partner. The Platform Operator holds no manual override authority to grant Terminal 3 access outside of this secure verification pathway.</p>
                        </div>
                        ...
                        <p className="mb-3 text-sm">The designation of Terminal 3 as a <strong>"Verified Domain"</strong> constitutes a <strong>Point-in-Time Temporal Snapshot</strong> only. The digital verification records and status indicators rendered within this domain reflect exclusively the historical verification state of the User's credentials at the exact timestamp of the last successful independent registry query sequence executed by the Platform's designated verification partner.</p>
                            ...
                            <table className="w-full text-sm border-collapse">
                                <tbody>
                                    {[
                                        ...
                                        ['(d) Statutory Investigation & Regulatory Legal Hold Override', 'Notwithstanding the automated TTL zero-out and destructive memory deallocation protocols set forth in Section 3.5(b), the Platform infrastructure incorporates an overriding administrative flag designated as the "Regulatory Safety Break (Legal Hold Mode)." Upon service of an official subpoena, production order, or evidence preservation notice issued by a competent state accident investigation authority — including but not limited to the Transport Safety Investigation Bureau of Singapore (TSIB), the National Transportation Safety Board (NTSB), the Air Accidents Investigation Branch (AAIB), or any equivalent Annex 13 investigation tribunal — the Platform Operator shall programmatically freeze all automated purging, cache pruning, TTL zero-out, and account-deletion cycles for any specified User UUIDs or enterprise routing sessions identified in the preservation notice. Once the Regulatory Safety Break is initialised, historical state metadata, timestamp logs, and token provenance records for the identified profiles shall be preserved in an immutable, read-only audit log container to maintain an uncorrupted chain of custody for state investigators. The User and all subscribing enterprises explicitly acknowledge that execution of a statutory legal hold constitutes full compliance with international treaty obligations under ICAO Annex 13, and the Platform Operator disclaims all commercial, contractual, or privacy liability to affected parties for restricting automated deletion or preserving profile routing indicators under a valid sovereign regulatory preservation directive. The zero-knowledge architecture is preserved under this protocol — the Platform holds and produces only infrastructure metadata (token issuance timestamps, signing key fingerprints, Terminal permission state logs); raw credential content, logbook data, and identity payloads were never held by the Platform and cannot be produced.'],
                                    ].map(([label, desc], i) => (
                                        <tr key={String(label)} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50'}>
                                            <td className="px-4 py-3 border border-slate-200 font-semibold text-slate-700 w-52 text-xs whitespace-nowrap align-top">{label}</td>
                                            <td className="px-4 py-3 border border-slate-200 text-slate-600 text-xs">{desc}</td>
                            ...
                            <div className="min-w-max text-xs space-y-2">
                                ...
                                <div className="flex items-center gap-3 justify-center">
                                    <div className="border-2 border-slate-500 rounded-lg px-4 py-2 bg-slate-100 text-center w-52">
                                        ...
                                        <p className="text-slate-600 text-[10px]">All records under compromised key</p>
                                    </div>
                                    <div className="text-slate-400 font-bold">→</div>
                                ...
                                <div className="text-center text-slate-300 font-bold mt-1">— — — — — — — — — — — — — — —</div>
                                <div className="text-center text-slate-400 text-[10px] font-bold uppercase tracking-wider">S3.5(d) Override Path — Annex 13 Legal Hold</div>
                                <div className="flex items-center gap-3 justify-center">
                                    <div className="border-2 border-indigo-400 rounded-lg px-4 py-2 bg-indigo-50 text-center w-52">
                                        <p className="font-black text-indigo-800 text-[10px] uppercase">Sovereign Production Order</p>
                                        <p className="text-indigo-600 text-[10px]">TSIB / NTSB / AAIB / Annex 13 tribunal</p>
                                    </div>
                                    <div className="text-slate-400 font-bold">→</div>
                                    <div className="border-2 border-emerald-500 rounded-lg px-4 py-2 bg-emerald-50 text-center w-52">
                                        <p className="font-black text-emerald-800 text-[10px] uppercase">Regulatory Safety Break</p>
                                        <p className="text-emerald-600 text-[10px]">All automated purging suspended</p>
                                        <p className="text-emerald-500 text-[10px]">Metadata → immutable audit container</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="border-l-4 border-indigo-400 bg-indigo-50 rounded-r-xl px-5 py-3 mb-2">
                            <p className="text-indigo-800 text-xs font-bold uppercase tracking-wide mb-1">Zero-Knowledge Preservation — Annex 13 Compliance</p>
                            <p className="text-indigo-700 text-xs leading-relaxed">The Regulatory Safety Break preserves infrastructure metadata only: token issuance timestamps, signing key fingerprints, Terminal permission state transitions, and webhook receipt logs. Raw credential content, logbook records, and identity payloads were never held by the Platform and cannot be produced under any preservation order. The zero-knowledge architecture remains fully intact under legal hold execution.</p>
                        </div>
                    </section>
                            ...
                            <table className="w-full text-sm border-collapse">
                                <tbody>
                                    {[
                                        ...
                                        ['Local Environment-Aware Alignment', 'The pilot\'s private dashboard locally evaluates these public criteria against the user\'s encrypted, structured container data — processed strictly inside the client-side browser runtime space.'],
                    ...
                    <section>
                        ...
                        <div className="space-y-2 mb-4">
                            {[
                                { step: '1', label: 'Profile Creation & Recognition+ Membership', tier: 'Terminal 1 · Membership Tier', color: 'slate', desc: 'The pilot initialises their non-custodial decentralized container and settles the Recognition+ membership fee. The platform issues a personal invite code for the pilot to save and share. The pilot\'s account is immediately listed as a Recognition+ member. Self-claimed aviation metadata (e.g., educational status, estimated flight hours, training records) is tokenised into standardised cryptographic credential format inside the browser environment.' },
                                { step: '2', label: 'Verification Trigger — Now or Later', tier: 'User-Initiated Trigger', color: 'blue', desc: 'The pilot is prompted to verify credentials immediately or defer to a later date. Recognition+ membership privileges remain active regardless of verification status. When the pilot elects to proceed, the system presents available independent third-party verification providers for selection.' },
                                { step: '3', label: 'Provider Selection & Document Submission', tier: 'Adaptive Regional Verification', color: 'amber', desc: '' },
                                { step: '4', label: 'Credential Issuance & Platform Triangulation', tier: 'Terminal 3 Clearance', color: 'emerald', desc: 'Upon validation, the independent verification entity issues a permanent Verified Credential Token directly into the pilot\'s decentralized container and transmits a binary confirmation signal to the platform. The platform reads this signal, cryptographically signs a Terminal 3 Access Record, and unlocks the respective verified pathway registries matching the pilot\'s verified tier.' },
                                        ...
                                        <div className="flex-1">
                                            ...
                                            {step === '3' ? (
                                                <div className="text-xs text-slate-600">
                                                    <p className="mb-2">The pilot selects an independent third-party verification provider from the Platform\'s curated partner list. The provider receives a secure webhook notification and the pilot uploads documents directly to the provider\'s secure portal. The provider reviews the submission and, upon readiness, notifies the Platform to trigger invoicing for the verification service.</p>
                        ...
                        <div className="bg-red-50 border-2 border-red-300 rounded-xl px-5 py-4 mb-5">
                            ...
                            <p className="text-red-800 text-xs leading-relaxed mb-2">You are strictly required to audit, verify, and guarantee the absolute accuracy of all documents you submit to your selected verification provider — including photographs of licences, medical certificates, logbook pages, and physical logbook photographs. These documents must be genuine, unaltered, and must accurately reflect your true aviation credentials, flight hours, and regulatory status.</p>
                            <p className="text-red-800 text-xs leading-relaxed mb-2">The verification provider will review your submitted documents and, upon confirming readiness to proceed, will notify the Platform to issue an invoice for the verification service. <strong>Once the provider begins active processing, the verification service fee is non-refundable</strong>. If your screening reveals revoked licences, lapsed medical checks, or invalid certifications, the verification fee remains payable in full and your submission will return a failed verification status, preventing access to Terminal 3.</p>
                            ...
                            <div className="min-w-max text-xs space-y-3">
                                <div className="flex items-center gap-2 justify-center">
                                    ...
                                    <div className="border-2 border-amber-300 rounded-lg px-3 py-2 bg-amber-50 text-center w-48">
                                        ...
                                        <p className="text-amber-600 text-[10px]">Verification service fee applies</p>
                                    </div>
                                    <div className="text-slate-400 font-bold">→</div>
                            ...
                            <table className="w-full text-sm border-collapse">
                                <tbody>
                                    {[
                                        ...
                                        ['(d) Mandatory Non-Disclosure, Anti-Leakage Bound & Analog Capture Acknowledgement', 'All documentation exposed within Stage 2 is explicitly deemed Proprietary Intellectual Property of the issuing aviation enterprise. The User is structurally barred from extracting, downloading, copying, or screenshotting the Stage 2 interface payload. Any unauthorised disclosure, public dissemination, or competitive leakage of these financial terms by the User constitutes a material breach of these Terms, resulting in immediate infrastructure termination and absolute legal liability localised strictly between the User and the affected enterprise. The Platform Operator bears zero liability for any downstream contractual, labour, or bond-enforcement dispute arising between the User and the enterprise from the contents of Stage 2 documentation. The subscribing aviation enterprise explicitly acknowledges that while the Platform deploys server-side rendering controls and browser-level security parameters to disrupt digital data extraction, the Platform cannot prevent physical analog capture of a rendering terminal screen by external hardware (including but not limited to mobile device photography). The enterprise accepts this inherent environmental risk of digital data display on a web runtime and assumes sole responsibility for the physical-environment security of any terminal used to access Stage 2 documentation.'],
                    ...
                    ══════════════════════════════════════════════ */}
                    <section>
                        <h3 className="font-semibold text-slate-800 mb-2">5.7 Hybrid Enterprise Employment Screening &amp; Routing Protocol (Stage 3)</h3>
                        <p className="mb-3 text-sm">The Platform distinguishes between the initial infrastructure onboarding check (the User-funded credential validation layer defined in Section 5.1) and advanced commercial background checks required for corporate recruitment pipelines, designated as <strong>Stage 3: Enterprise-Sponsored Employment Screening</strong>. Stage 3 supports two mutually exclusive routing paths selected by the subscribing aviation enterprise.</p>
                        <div className="overflow-x-auto mb-4">
                            <table className="w-full text-sm border-collapse">
                                <tbody>
                                    {[
                                        ...
                                        ['(b) Dual-Routing Framework (Native vs. Enterprise-Proprietary)', 'Advancing to Stage 3 requires an independent, explicit cryptographic consent action initiated by the User via hardware passkey signature, entirely separate from the consents executed under Sections 5.5 and 5.6. Upon execution of consent, the processing lifecycle utilises one of two mutually exclusive routing paths selected by the subscribing aviation enterprise: Path A — Native Integrated Routing ("In-House"): the verification request is routed through the Platform\'s designated independent verification integrations; all transactional surcharges are billed to the enterprise profile and settled via the automated, non-custodial smart contract infrastructure defined in Section 7.5. Path B — Enterprise-Proprietary Routing ("Airline\'s Own"): the verification request, data payload, and consent parameters are exported directly to the subscribing aviation enterprise\'s proprietary external background screening vendor; the Platform functions solely as an outbound cryptographic transport layer for this handoff and assumes zero liability for any processing, compliance, or data protection obligations past the outbound export boundary.'],
                                        ['(c) Allocation of Extraterritorial Screening Costs', 'All operational costs, provider fees, and systemic processing surcharges associated with Stage 3 screenings are borne exclusively by the subscribing aviation enterprise. Under Path A, fees are processed via the Platform\'s non-custodial architecture as defined in Section 7.5; the Platform Operator never transiently holds, escrows, or handles these funds. Under Path B, financial settlement occurs entirely off-platform between the enterprise and its proprietary vendor; the Platform Operator assumes null liability for processing failures, currency fluctuations, or settlement disputes occurring within the enterprise\'s private payment networks.'],
                                        ['(d) Absolute Liability Exclusion for Proprietary Systems', 'For screenings executed under Path B, the Platform Operator explicitly disclaims all operational, legal, and statutory data protection liability. The subscribing aviation enterprise assumes sole, unmitigated accountability as the exclusive Data Controller for the security, processing hygiene, regulatory compliance, and outcomes generated by its proprietary external screening systems. Any data breach, transmission failure, or regulatory non-compliance occurring past the Platform\'s outbound export boundary resides fundamentally outside the Platform Operator\'s control, and the enterprise shall fully indemnify and hold harmless the Platform Operator against any resulting claims, regulatory investigations, or enforcement actions.'],
                                        ['(e) Disclaimer of Employment Adjudication & Algorithmic Neutrality', 'Under both Path A and Path B, the Platform Operator functions strictly as a neutral technical routing interface for the transmission of Stage 3 consent parameters and tokenized binary status flags only. The Platform Operator does not author, inspect, analyse, or interpret the raw contents of employment screening results under either routing path. Final hiring outcomes, contract awards, or pathway rejections resulting from favourable or adverse Stage 3 findings reside strictly within the autonomous data domains of the User and the subscribing aviation enterprise, governed by the Tripartite Data Controller Framework (Section 17.1). The Platform Operator does not make, influence, or recommend employment decisions. All data indicators displayed on the enterprise dashboard — including but not limited to Live Telemetry Status Indicators, credential verification beacons, and profile match scores — are neutral, mathematically computed informational outputs applied uniformly across all profiles without reference to age, nationality, sex, race, religion, or any other protected characteristic under the Singapore Tripartite Guidelines on Fair Employment Practices (TAFEP) or any applicable employment discrimination statute. Any employment decision made by a subscribing aviation enterprise on the basis of platform-displayed data is an autonomous decision of that enterprise acting as an independent Data Controller. The enterprise bears sole and exclusive legal responsibility for ensuring its use of platform data in its recruitment process complies with all applicable employment, anti-discrimination, and labour laws. The Platform Operator cannot be impleaded as a joint tortfeasor in any employment discrimination claim arising from the enterprise\'s independent hiring decisions.'],
                                        ['(f) Screening Status Containment — Single-Enterprise Scope', 'A Stage 3 screening result — whether cleared or adverse — is scoped exclusively to the initiating aviation enterprise\'s private dashboard instance under both routing paths. An adverse finding for one enterprise does not propagate, flag, or modify the User\'s profile visibility, Terminal 3 status, or credential beacon across any other active corporate dashboard on the platform. Each Stage 3 screening is a discrete, bilaterally scoped data transaction between the User and the requesting enterprise only. The Platform Operator maintains zero cross-enterprise aggregation of Stage 3 outcomes.'],
                                    ].map(([label, desc], i) => (
                                        <tr key={String(label)} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50'}>
                                            <td className="px-4 py-3 border border-slate-200 font-semibold text-slate-700 w-52 text-xs whitespace-nowrap align-top">{label}</td>
                                            <td className="px-4 py-3 border border-slate-200 text-slate-600 text-xs">{desc}</td>
                        ...
                        {/* Stage 3 dual-routing flow diagram */}
                        <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 mb-4 overflow-x-auto">
                            <div className="min-w-max text-xs space-y-2">
                                <div className="flex items-center gap-3 justify-center">
                                    <div className="border-2 border-blue-300 rounded-lg px-4 py-2 bg-blue-50 text-center w-56">
                                        <p className="font-black text-blue-800 text-[10px] uppercase">Pilot Executes Stage 3 Consent</p>
                                        <p className="text-blue-600 text-[10px]">Independent passkey signature</p>
                                        <p className="text-blue-500 text-[10px]">Separate from S5.5 &amp; S5.6</p>
                                    </div>
                                    <div className="text-slate-400 font-bold">→</div>
                                    <div className="border-2 border-amber-400 rounded-lg px-4 py-2 bg-amber-50 text-center w-52">
                                        <p className="font-black text-amber-800 text-[10px] uppercase">Airline Routing Selection</p>
                                        <p className="text-amber-600 text-[10px]">Path A or Path B</p>
                                    </div>
                                </div>
                                <div className="text-center text-slate-400 font-bold">↓ routing fork ↓</div>
                                <div className="flex items-start gap-4 justify-center">
                                    <div className="border-2 border-emerald-400 rounded-lg px-3 py-2 bg-emerald-50 text-center w-52">
                                        <p className="font-black text-emerald-800 text-[10px] uppercase">Path A — In-House</p>
                                        <p className="text-emerald-600 text-[10px]">Platform integrated partner</p>
                                        <p className="text-emerald-700 text-[10px]">Non-custodial billing (S7.5)</p>
                                        <p className="text-emerald-600 text-[10px]">Platform: S18 immunity applies</p>
                                    </div>
                                    <div className="text-slate-400 font-bold mt-4">|</div>
                                    <div className="border-2 border-slate-400 rounded-lg px-3 py-2 bg-slate-100 text-center w-52">
                                        <p className="font-black text-slate-700 text-[10px] uppercase">Path B — Airline's Own Vendor</p>
                                        <p className="text-slate-600 text-[10px]">Outbound cryptographic handoff</p>
                                        <p className="text-slate-600 text-[10px]">Off-platform settlement</p>
                                        <p className="text-red-600 text-[10px] font-black">Airline = 100% Data Controller</p>
                                    </div>
                                </div>
                                <div className="text-center text-slate-400 font-bold">↓ binary status beacon only — both paths ↓</div>
                                <div className="flex items-center gap-3 justify-center">
                    ...
                    ══════════════════════════════════════════════ */}
                    <section>
                        <h2 className="text-2xl font-bold text-slate-900 mb-4">7. Limitation of Liability &amp; Membership / Verification Fee Terms</h2>

                        <h3 className="font-semibold text-slate-800 mb-2">7.1 Membership Fee &amp; Verification Service Invoicing</h3>
                        <div className="bg-amber-50 border border-amber-200 rounded-xl px-5 py-4 mb-4">
                            <p className="text-amber-800 text-xs font-black uppercase tracking-wide mb-2">Recognition+ Membership Fee</p>
                            <p className="text-amber-900 text-xs leading-relaxed">The Recognition+ membership fee is a <strong>yearly subscription</strong> that grants access to the Platform's invite-code system, profile creation tools, and membership-tier privileges. <strong>This fee is non-refundable</strong> regardless of whether the User subsequently elects to verify credentials or defers verification indefinitely. The membership fee is distinct from and independent of any verification service charges.</p>
                        </div>
                        <p className="mb-3 text-sm"><strong>Verification Service Invoicing:</strong> When the User elects to verify credentials, the User selects an independent third-party verification provider from the Platform's curated list. The provider receives the User's documents directly and, upon confirming readiness to proceed, notifies the Platform. The Platform then invoices the User for the verification service fee. <strong>The verification service fee is non-refundable once the provider has begun active processing of the submitted documents.</strong></p>
                        <p className="mb-4 text-sm">In the event that a regional screening provider discovers discrepancies, expired credentials, or administrative flags that result in a denial of Terminal 3 access after processing has commenced, <strong>the verification service fee remains non-refundable and non-creditable</strong>. The Platform Operator shall not be held liable for fees forfeited due to a pilot's failure to properly self-audit their data prior to submission. The User is encouraged to review all documents for accuracy before authorising the provider to begin processing.</p>
                        ...
                        <p className="mb-3 text-sm">The USD $50.00 aggregate cap defined in Section 7.4 applies to individual User accounts operating on the Platform's free or standard-fee tier. Subscribing aviation enterprises operating under a paid B2B enterprise subscription agreement are subject to a separate, tiered liability ceiling as defined in this section, reflecting the commercial nature of the enterprise relationship and satisfying the reasonableness standard under the Singapore Unfair Contract Terms Act as applied to sophisticated commercial parties. The reasonableness of this ceiling is assessed at the time the enterprise subscription contract is formed — not against consequential operational losses arising in hindsight from a worst-case failure scenario. The enterprise acknowledges that the subscription fee is priced as a data infrastructure access service and that the liability ceiling reflects the commercial scope of that service as understood at contract formation.</p>
                        <div className="overflow-x-auto mb-4">
                            <table className="w-full text-sm border-collapse">
                                <thead>
                                    <tr className="bg-slate-900 text-white">
                        ...
                        <div className="border-l-4 border-red-400 bg-red-50 rounded-r-xl px-5 py-3 mb-4">
                            ...
                            <p className="text-red-700 text-xs leading-relaxed">Regardless of subscription tier, the Platform Operator's liability ceiling under this section <strong>never includes</strong>: (i) indirect, consequential, or punitive damages; (ii) loss of profits, revenue, or business opportunity; (iii) reputational harm, brand diminution, or trade libel claims; (iv) losses arising from a third-party verification partner's security failure (governed by Section 5.7(g) and Section 3.5); (v) losses arising from the enterprise's own failure to maintain mandatory independent crew verification obligations under ICAO Annex 6 or applicable national air operator certification requirements; (vi) losses arising from the enterprise's own employment decisions, algorithmic hiring practices, or discrimination claims asserted by pilots against the enterprise arising from the enterprise's use of platform data in its recruitment process. These exclusions apply to the maximum extent permitted by applicable law and survive any finding of fundamental breach under common-law contract principles.</p>
                        </div>

                        <h3 className="font-semibold text-slate-800 mb-2">7.4B Stage 2 Enterprise Insurance Obligation</h3>
                        <p className="mb-3 text-sm">Access to the Stage 2 Asymmetric Enterprise Vault (Section 5.6) requires the subscribing aviation enterprise to maintain, at its own expense, adequate professional indemnity and trade secret insurance coverage for any confidential, proprietary, or commercially sensitive documents uploaded into the Stage 2 session vault. The Platform Operator provides the technical isolation infrastructure for the vault; it does not insure the enterprise's own intellectual property or confidential documentation against loss, exposure, or misappropriation arising from circumstances outside the Platform's direct technical control.</p>
                        <div className="border-l-4 border-amber-400 bg-amber-50 rounded-r-xl px-5 py-3 mb-4">
                            <p className="text-amber-800 text-xs font-bold uppercase tracking-wide mb-1">Enterprise Self-Insurance Acknowledgement — Stage 2 Vault</p>
                            <p className="text-amber-700 text-xs leading-relaxed">By uploading confidential documentation into the Stage 2 vault, the enterprise explicitly acknowledges: (i) it has assessed the risk of uploading commercially sensitive materials to a third-party infrastructure platform and elected to do so; (ii) it maintains or will maintain appropriate professional indemnity, trade secret, and cyber-risk insurance commensurate with the commercial value of the materials uploaded; (iii) the Platform Operator's liability ceiling under Section 7.4A applies as the exclusive financial remedy available to the enterprise for any Stage 2 technical failure attributable to the Platform; (iv) the enterprise's insurance coverage, not the Platform's liability ceiling, constitutes the primary financial protection for the enterprise's own uploaded intellectual property. The Platform Operator's liability ceiling is commercially reasonable given that the enterprise retains independent insurance capacity for its own high-value confidential assets.</p>
                        </div>
                            ...
                            <table className="w-full text-sm border-collapse">
                                <tbody>
                                    {[
                                        ...
                                        ['Comparable Protection Standard & Automated DTA Binding', 'The Platform Operator enforces data protection requirements through its service agreements, ensuring that any overseas recipient provides a standard of protection to the transferred personal data that is comparable to the protection under the PDPA. To satisfy Section 26 of the Singapore PDPA 2012, the Platform Operator enforces mandatory, automated Data Transfer Agreements (DTAs) within its B2B enterprise onboarding protocols. Every overseas subscribing aviation enterprise, by initialising an operational profile on the Platform, contractually binds itself to afford all received tokenized metadata a standard of protection that is fully comparable to the protections mandated under the Singapore PDPA. The Platform Operator\'s obligation is restricted to the execution of these contractual transfer instruments; the Platform Operator assumes no liability for the subsequent actions, omissions, or regulatory breaches committed by the overseas recipient after the DTA-governed transfer boundary.'],
                    ...
                    <section>
                        ...
                        <ul className="space-y-3 mb-4 text-sm text-slate-600">
                            <li><strong>Instantaneous Terminal 3 Egress:</strong> Upon receipt of a live API revocation or failed trust-anchor handshake callback, the infrastructure layer instantly updates the credential status to "Revoked" across all synchronised database engine layers. The corresponding Terminal 3 Access Record is cryptographically invalidated via immediate TTL zero-out, dropping the profile back to Terminal 1 (Baseline) functions within 60 seconds of the network callback event.</li>
                    ...
                    <section>
                        ...
                        <h3 className="font-semibold text-slate-800 mb-2">14.2 Zero-Data Verification Architecture</h3>
                        <p className="mb-3 text-sm">The Platform does not process, store, or persist any original verification documents or raw background-check payloads. All document uploads occur directly between the User and their independently selected verification provider.</p>
                        <div className="overflow-x-auto mb-4">
                            <table className="w-full text-sm border-collapse">
                                <tbody>
                                    {[
                                        ['User Consent & Provider Terms Acknowledgement', 'Before initiating any document upload, the User must actively select an independent third-party verification provider and electronically sign a consent record acknowledging the provider\'s terms of service and privacy policy. This consent step is executed within the Platform\'s interface but binds the User contractually to the selected provider, not to the Platform Operator.'],
                                        ['Document Accuracy Attestation', 'The User is required to declare under penalty of perjury or applicable local law that every document submitted — including but not limited to photographs of licences, medical certificates, logbook pages, training records, and physical logbook photographs — is genuine, unaltered, and accurately reflects the User\'s true aviation credentials, flight hours, and regulatory status at the time of upload. False, misleading, or fraudulent submissions may result in immediate verification failure, permanent platform ban, revocation of all issued credentials, and referral to relevant civil aviation authorities or law enforcement.'],
                                        ['No Document Ingress to Platform Servers', 'License, medical, training, and employment documents are uploaded by the User directly to the third-party verification provider chosen by the User. These documents never transit through, are not received by, and are not stored on any Platform server or database. The Platform Operator has no technical ability to access, view, or retain the original document files.'],
                                        ['Verifiable Credential Receipt Only', 'Upon completion of a verification check, the Platform receives only a cryptographically signed Verifiable Credential (VC) attestation — a lightweight proof-of-status — from the verification provider via webhook. This VC contains no raw document data, no biometric images, and no free-text background-check narratives. It is limited to structured claims (e.g., licence validity status, medical class, verification timestamp) required for Platform access control.'],
                                        ['Result Delivery Direct to User', 'The full verification report, including detailed findings, is transmitted directly from the third-party provider to the User via email or the provider\'s own secure portal. The Platform does not receive, store, or forward these detailed results.'],
                                        ['No Transient Cache or Ephemeral Storage', 'Because no document payloads enter Platform infrastructure, there is no transient file cache, temporary upload buffer, or ephemeral storage layer requiring TTL-based deletion. The sole data artifact retained by the Platform is the signed VC attestation, which is stored as a permanent credential in the User\'s wallet for downstream airline sharing.'],
                                    ].map(([label, desc], i) => (
                                        <tr key={String(label)} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50'}>
                                            <td className="px-4 py-3 border border-slate-200 font-semibold text-slate-700 w-52 text-xs whitespace-nowrap align-top">{label}</td>
                                            <td className="px-4 py-3 border border-slate-200 text-slate-600 text-xs">{desc}</td>
