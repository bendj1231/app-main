    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="w-full flex gap-6 pl-4 pr-6 py-8">
        ...
        <article className="max-w-4xl mx-auto flex-1 min-w-0">
          ...
          <ul className="space-y-2 mb-6">
            {[
              { n: '1', t: 'No central directory', d: 'Job posts scattered across social media, professional networks, generic boards. No single source of truth.' },
              { n: '2', t: 'Opaque requirements', d: 'Exact hours, ratings, and competency expectations rarely published clearly.' },
              { n: '3', t: 'No self-alignment tool', d: 'Pilots cannot compare their profile against requirements before applying.' },
              { n: '4', t: 'Outdated information', d: 'Requirements change; pilots align to old job posts without knowing.' },
              { n: '5', t: 'No demand signals', d: 'Fleet expansion and type rating demand invisible. Pilots invest blindly.' },
              { n: '6', t: 'Unverified status', d: 'No differentiation between verified, current profiles and stale CVs.' },
            ].map((item) => (
              <li key={item.n} className="ml-6 text-slate-700 leading-relaxed flex items-start gap-2">
                <span className="text-red-500 mt-1 flex-shrink-0">→</span>
                <span><strong className="text-red-600">{item.n}. {item.t}</strong> — {item.d}</span>
